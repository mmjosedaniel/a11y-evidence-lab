...
Apply YAGNI to repository workflow as well as product code. Use only the roles, reviews, plan sections, or parallelism triggered by the current work. The existence of eleven role definitions is capacity, not a requirement to invoke them all. Do not add workflow telemetry merely because a later progress record may be useful; introduce the smallest progress artifact only when implementation creates a present reporting need.
