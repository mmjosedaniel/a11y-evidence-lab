...
```mermaid
flowchart LR
    A["Primary coordinator"] -->|"semantic events and correlation IDs"| D["Atomic local event files"]
...
Every invocation publishes one uniquely named JSON file under `logs/agent-flow-metrics/v2/events/`. The repository already ignores `logs/`, and the runtime location remains writable even when the Codex configuration layer is protected from shell writes. Files are written through an atomic same-directory replacement, so concurrent asynchronous hooks never append to a shared file and may safely finish out of order.

## Metric definitions

| Metric | Counted when | Not counted when |
|---|---|---|
| Correction | The coordinator records `correction_requested` with the same workflow, roadmap task, coherent work-slice cycle, phase, and role, plus a new correction lease ID, its terminal attempt-1 parent lease ID, and attempt 2, because a handoff was rejected or an actionable reviewer finding requires additional worker work | Initial assignments, attempts other than 2, clarifications with no new work, a new work slice, preflight routing, passing characterization, or a stopped/reclassified branch |
...
Only a coordinator-classified attempt-2 worker correction contributes to the correction metric. A new work slice, owner-directed change, preflight route, passing characterization, or reclassified assignment is not `correction_requested`, even when it follows a rejected handoff or review finding.

Workflow elapsed time measures the coordinator's recorded start-to-completion interval. Lease time measures assigned phases. Agent time comes from linked lifecycle hooks: each session/turn/agent start-stop pair is validated independently, then persistent-agent turns are aggregated by agent ID. Summed agent time can exceed workflow wall time when independent agents overlap, so the report keeps these scopes separate.
...
Every coordinator event requires the same workflow and task identity. Lease start, completion, and correction records also require the same cycle, lease, correction-parent, phase, attempt, agent, and role identity. Attempt-1 lease events omit the parent; attempt-2 lease events and their `correction_requested` record name the same distinct attempt-1 parent. A parent may identify only one correction child in the dataset. A token report for a lease-linked worker must match that task and role, while an identified primary report remains unlinked. Lifecycle pairs must match session, turn, agent, and role. When lifecycle and semantic lease observations share an agent ID, their reported roles must also match. A stable regression check belongs to only one cycle within a workflow.

Each distinct bounded Setup objective uses a fresh dedicated setup cycle ID created after the stable workflow ID and before that Setup assignment; only its attempt-2 correction preserves that ID. Each coherent work slice uses one cycle ID shared by its preflight context, characterization or Red, Green, and bounded corrections. The cycle ID is runtime correlation derived from an existing roadmap task, not a second task hierarchy. Preflight itself has no lease event, and a passing characterization is not a Red decision. The metrics schema correlates write-assignment identities but does not authorize their creation, reuse, classification, or evidence acceptance.

Exact logical replays are idempotent: aggregation keeps the first observation, counts it once, and reports the dataset-wide replay count. The validator rejects and the summary excludes contradictory Red decisions, conflicting copies of one logical event or token report, semantic-event identity conflicts, multiple correction children naming one parent, lifecycle-pair identity conflicts, reported lifecycle-to-semantic role mismatches for a shared agent ID, and a completion timestamp earlier than its start. These cross-checks compare reported fields; they do not independently attest agent identity or replace the guard's authoritative correction-lineage checks. An unpaired start or completion remains an explicit coverage gap because background hooks can be cancelled.
...
Retain this sidecar only for a bounded first-implementation pilot with named questions: whether the worker-first route produces corrections or false Reds often enough to justify its coordination cost, and whether lifecycle coverage is usable. No real lifecycle pair has yet been accepted as repository evidence, so runtime activation remains unproven. After trusting the current hook definition, run at least one matcher-covered worker or reviewer lifecycle and require a valid start/stop pair before claiming activation. At the first completed implementation work-slice barrier, inspect the resulting summary once. If capture is still absent or the observations do not inform a concrete workflow adjustment, disable the hooks and defer the metrics CLI rather than adding more telemetry. Metrics remain off the critical path throughout this pilot.

## Coordinator operation

Use stable, non-secret identifiers. Here `task_id` means the selected existing roadmap task ID. A practical pattern is `<ROADMAP_ID>-<YYYYMMDD>-<sequence>` for a workflow, `<workflow>-setup-01` or `<workflow>-slice-01` for a cycle, and `<cycle>-red-01` for a lease.
...
python -B .codex/metrics/agent_flow_metrics.py record `
  ...
  --task-id M1-03 `
  --cycle-id M1-03-20260828-01-slice-01 `
  --lease-id M1-03-20260828-01-slice-01-red-02 `
  --correction-parent-lease-id M1-03-20260828-01-slice-01-red-01 `
  --phase red `
  --attempt 2 `
  --agent-id AGENT_ID `
  --agent-type test_worker `
...
Close each lease with the same task, cycle, lease, correction-parent, phase, attempt, agent, and role fields plus `--event lease_completed`; omit the correction-parent field for attempt 1. A correction gets a new lease ID, carries its terminal attempt-1 parent on `correction_requested`, `lease_started`, and `lease_completed`, and uses attempt 2; never reuse the rejected lease or record attempt 3. Record `regression_confirmed` only after the workflow's failure-triage rule is satisfied and pass `--check-id` with the stable ID of the previously passing check. That check ID may be reported only once in a workflow and cannot be moved between cycles. Preserve the decisive baseline and failing commands in the ExecPlan or task evidence because the metric event does not replace that proof.
...
The eight-check self-test covers the privacy allowlist, valid non-blocking hook output, malformed input and storage failure, rejection of negative token values, out-of-order pairing, exact replay, conflicting events, invalid time transitions, workflow/task and correction/lease association, attempt-specific correction-parent presence and pairing, the one-child observation rule, linked token roles, lifecycle-to-semantic role agreement, persistent lifecycle turns, stable regression identity, metric counts, partial token coverage, and concurrent atomic publication.

The interactive `record` command rejects invalid semantic input so the coordinator can report a coverage gap. The lifecycle hook entry point instead fails open: it never changes the subagent's continuation behavior. `summary` excludes malformed, conflicting, or mis-associated events that the schema can detect and reports them; `validate` returns nonzero for malformed files, logical conflicts within semantic or lifecycle groups, reported lifecycle-to-semantic role mismatches for a shared agent ID, or invalid paired transitions. Exact replays remain valid but are counted once and reported.

## Privacy and limitations

Lifecycle events persist only the schema and event identity, observation time, source, session and turn IDs, agent ID and type, model, and permission mode. Semantic events add only coordinator-selected correlation IDs, nullable correction-parent lease ID, phase, attempt, reason code, and exact token counters.

Event publication is atomic and the tool treats each unique event path as append-only, but ordinary owner filesystem permissions can still modify or delete a published file. No content digest, read-only bit, or tamper-evident chain is provided. A valid-looking post-publication edit may therefore remain undetected; metrics are operational observations, never authority, implementation evidence, or a closure gate. The validator excludes a reported lifecycle/lease role mismatch; the coordinator should treat suspected file mutation or unverifiable identity as a coverage gap.
...
This is repository workflow tooling, not application source, a product test harness, or completion evidence for any roadmap task. It uses only the Python standard library and does not select or modify the TypeScript application toolchain governed by RD-002.
