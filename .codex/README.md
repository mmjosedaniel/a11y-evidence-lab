...
|---|---|---|
| Primary coordinating thread | `gpt-6-astra`, `xhigh` as the starting operator setting; another explicit owner-selected setting may be used | Owns routing, synthesis, guarded implementation coordination, authority reconciliation, and closure under the permission boundaries below. The operator selects the parent-session setting; custom-agent files do not enforce it. |
...
Use a small manual comparison on representative work before claiming an improvement. Keep task inputs, acceptance criteria, available tools, and effective effort comparable: first compare the previous model with Astra using unchanged instructions, then compare the original and revised instructions on the same model. Changing both at once cannot isolate the cause. The previous routing used Sol for the primary and the five now-Astra roles; ordinary role pins were unchanged.
