...
Default work-slice limits are one preflight, one coherent Red or characterization, one Green, and at most three write attempts per unchanged worker role/phase assignment chain: the initial turn, one ordinary correction, and one conditional final correction. Before attempt 3, the coordinator records progress or material new evidence, learning, the changed bounded action, expected benefit, and remaining authorization under the [correction rules](./execplan-implementation-workflow.md#corrections-and-exceptions). Each correction names its immediately preceding terminal parent, with one child per parent. Stop after unsuccessful attempt 3, the same decisive failure twice without material new evidence, two no-diff write handoffs, changed binding fields, or exhausted authorization. Stricter existing limits remain binding; replacement agents or new IDs cannot renew a budget. The [ADR-0024 amendment](../docs/architecture/decisions/ADR-0024-milestone-slice-tdd-with-independent-ownership.md#conditional-third-implementation-attempt) changes only the implementation-worker allowance. One review correction loop and all research/review budgets remain unchanged. More than three TDD cycles in one slice still requires rescoping instead of silent microcycling.

Review is risk-routed:

| Tier | Typical surface | Review route |
|---|---|---|
| `S0` | Documentation-only or deterministic mechanical work | Primary inspection and deterministic checks; no additional reviewer by default |
| `S1` | Ordinary bounded application work | Primary inspection and focused evidence; fresh `milestone_reviewer` only for a named unresolved correctness/coverage risk or owner request |
| `S2` | A consequential changed interface, state transition or cross-boundary integration | Fresh `independent_reviewer`, normally covering the integrated change |
| `S3` | A changed security, irreversible-data, concurrency, locking, recovery, custom serialization/integrity/identity or cross-platform boundary | Fresh `critical_reviewer`, normally covering the integrated change; before consequential reliance when needed |

The [model policy](#model-and-reasoning-policy) owns each role's model and effort. Name the affected boundary and concrete risk; a path, keyword or unchanged use of an existing mechanism does not trigger a higher tier. An unresolved Blocker or Major after ordinary review, or explicit project-owner direction, also triggers critical review. The primary and any triggered reviewer apply the common [changed-surface quality baseline](./execplan-implementation-workflow.md#changed-surface-quality-baseline); the frontend route retains reuse and browser evidence without another reviewer. Exact-path authorization contains writes but does not prove cohesive responsibility placement.

The [review procedure](./execplan-implementation-workflow.md#work-slice-and-final-review) owns review timing and reuse: consolidate S2/S3 coverage at integration unless consequential reliance requires an earlier review. Require a separate fresh final reviewer only for an unreviewed integration risk, materially invalidated coverage or an explicit owner requirement. All required coverage must pass before closure. Follow the [verification cadence](./execplan-implementation-workflow.md#verification-cadence): focused development checks, one full suite after integration, and affected checks for subsequent changes. Reuse fresh results instead of repeating suites at every handoff. [ADR-0024's amendment](../docs/architecture/decisions/ADR-0024-milestone-slice-tdd-with-independent-ownership.md#proportional-verification-and-review) preserves active authority, task-specific gates and consumed budgets.
