...
Apply the [small, cohesive modules rule](../AGENTS.md#small-cohesive-modules-from-the-start) to the first implementation handoff, not only to a later refactor request. Check that React composition does not absorb distinct forms, lists, or detail-region responsibilities, and that backend coordination does not absorb unrelated validation, domain, or I/O responsibilities. A material current-scope decomposition defect is a finding to resolve through the existing correction or reconciliation route before acceptance, not an automatic future-cleanup item. This adds no reviewer, phase, or permission to expand the task or lease.
