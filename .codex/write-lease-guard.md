...
- `start` accepts attempts `1` and `2`. It permits `test_worker` phases `red` or `evidence`, `code_worker` phases `setup` or `green`, and only the `green` phase for `frontend_code_worker`; values are lowercase and case-sensitive. The work-slice workflow uses `evidence` for a passing characterization, `red` for a missing or regressed contract, `setup` for independent standard-profile setup, and `green` for implementation plus optional same-turn Refactor. There is no standalone Refactor or implementation-worker evidence lease. The frontend role remains restricted to the conditional `frontend-visual` Green route; the semantic profile and visual capsule stay in the packet because the guard validates only worker/phase compatibility. Before attempt 2, the prior lease must be terminal and the coordinator must reconcile the tree and last accepted barrier. The guard pins attempt and owner but does not decide whether a correction is semantically valid.
