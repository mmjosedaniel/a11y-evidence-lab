...
The automatic write-lease guard makes the machine-verifiable projection of a worker-first path contract enforceable. The complete semantic assignment lives in [`Milestone Assignment Packet v2`](./execplan-implementation-workflow.md#milestone-assignment-packet-v2); the guard pins its workflow identity, work-slice identity through `cycle_id`, correction-parent lineage, path scope, repository endpoint baseline, logical Git index, `HEAD` object and symbolic ref, and the selected Git settings and ignore controls described below. The primary coordinator captures that immutable projection immediately before each write turn and terminally closes the exact lease after that turn stops. A handoff barrier may advance only when closure returns a compliant receipt.
...
The isolated suite checks assignment identity at both trust boundaries and correction lineage at start: invalid attempts and worker/phase pairs fail before `start` creates state; digest-valid stored contracts with invalid assignment identities or parent fields fail when reloaded; and complete `1 -> 2 -> 3` lifecycles, missing, unknown, self, nonterminal, skipped, mismatched, and duplicate parents are exercised. It also checks competing third children and an intervening completed child before reservation, requiring the later lineage recheck to reject the stale contender and roll back only its unpublished state. The JSON result lists the grouped checks that ran.
...
- `start` accepts attempts `1`, `2`, and `3`. Attempt 1 must not name a correction parent. Attempts 2 and 3 must pass `--correction-parent-lease-id` naming a terminal lease from the immediately preceding attempt: `1 -> 2 -> 3`. Workflow, task, work slice, phase, worker role, and path scope must match, and each parent may have only one child. The guard checks lineage before and after reserving the active lease and rejects skipped predecessors, duplicate children, and attempt 4. Before an attempt-3 start, the coordinator must satisfy the [conditional correction rules](./execplan-implementation-workflow.md#corrections-and-exceptions); successful guard validation does not establish progress, justify another attempt, or renew a budget.
...
- The worker contract prohibits every Git write operation or metadata mutation, including staging, committing, pushing, creating or changing tags, refs, or branches, stashing, altering worktrees, remotes, repository configuration, hooks, or other `.git` state. The guard detects logical Git-index content or flag drift, `HEAD` object drift, symbolic branch/detachment drift, and the selected settings and ignore-control changes described below. Other Git metadata, including tags, remains prohibited but is outside the guard's proof boundary.
...
Runtime data lives under the already ignored `logs/agent-flow-leases/v2/` directory. A guard contract stores identifiers including owner, attempt, and nullable correction-parent lease ID, normalized scope, repository identity, `HEAD` object and symbolic reference, selected Git-state and ignore-control digests, path names, and SHA-256 content digests. It intentionally does not store the correction reason, the full Milestone Assignment Packet, prompts, messages, command output, secrets, or file contents. Contracts and receipts are write-once and digest-pinned; this is tamper-evident workflow state, not an operating-system security boundary against a malicious same-user process.

The third-attempt extension retains schema v2, canonical serialization, digest rules, and existing attempt-1/2 records without migration or rewriting. An older guard that accepts only attempts 1/2 cannot read a new attempt-3 contract and may also fail while scanning it during an unrelated correction start. After publishing attempt 3, retain a compatible reader; deleting or rewriting runtime state is not a downgrade procedure.

Lineage validation accepts a valid terminal parent receipt of either outcome; a `violated` receipt still requires coordinator reconciliation before later work. Owner identity is recorded but is not a matched lineage field. Stored-contract readback validates that record's shape and digest, not its complete ancestry. The guard does not detect a semantic budget reset disguised as fresh workflow or work-slice IDs, and it does not evaluate third-attempt justification. These remain explicit coordinator duties.
