...
import { readProviderInvocation, type GenerationOutcome } from './generation-contract.ts';
...
export async function executeGeneration(options: StageInput): Promise<GenerationOutcome> {
  const outcome = await executeGenerationOperation({
    expiresAt: options.expiresAt, signal: options.signal, providerContext: options.providerContext, adapter: options.adapter,
    async admit() {
  ...
  const invocation = observation && readProviderInvocation({ ...observation.adapterConfiguration,
