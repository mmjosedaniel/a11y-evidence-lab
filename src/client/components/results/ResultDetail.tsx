...
export function ResultDetail({ idPrefix, result, providerContext, guidance, generation }: ResultDetailProps): ReactElement {
  ...
  return <section className="finding-detail" role="region" aria-label={`${result.label} evidence`}>
    ...
    {result.kind === 'finding' && result.item.state === 'proposal-pending-review' && view &&
      <ProposalDetail proposal={result.item.result} view={view} />}
  </section>;
}
