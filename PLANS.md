...
For worker-first implementation, record assignment identity, responsible worker, terminal lease result, preflight route or justified `TDD: Not applicable` setup route, exact Red and Green commands when applicable, task-level validation, coordinator acceptance, any exceptional direct coordinator test correction, evidence invalidation or reuse under an unchanged fingerprint, and the risk-routed reviewer verdict. A packet, worker summary, or guard result cannot replace inspection of the actual diff and task-authoritative validation.
