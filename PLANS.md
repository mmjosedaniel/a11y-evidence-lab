...
Before each behavior-bearing work slice completes, audit affected tests, fixtures, mocks, helpers, snapshots, skipped tests, and focused-test markers. Summarize material coverage changes and gaps without an entry per unchanged assertion. Add focused tests for changed behavior, uncovered contract gaps, and reproduced defects; retain existing tests unless specific redundancy or invalidity is established. Follow the [verification cadence](.codex/execplan-implementation-workflow.md#verification-cadence): focused checks during development, one complete authoritative suite after integration, and affected checks for later changes. Reuse that evidence at closure; a separate phase handoff does not require another full run. Preserve required strict/build and task-specific proof. If no executable suite applies, record the replacement evidence instead of inventing a command.
