...
Run data stays in the ignored `data/runs/<run-id>/run.json` tree. Reads never repair invalid records, promote staging residue, or automatically resume interrupted work. The parent completed/failed scan state is terminal. M2-02 permits only selected-Finding retrieval activation and completion/failure updates inside a completed aggregate; completed scan evidence and sibling data stay immutable. Later support, generation, review and comparison updates remain their owning tasks. No backup, hidden copy, sweep, or synchronization mechanism is added.
...
Internal slice-A APIs are [loadCorpusCatalog/createCorpusDocuments](src/server/retrieval/corpus-catalog.ts), [createFindingQuery](src/server/retrieval/finding-query.ts), and [validateRetrievalResult](src/server/retrieval/retrieval-contract.ts). The loader accepts only the fixed normalized corpus identities and preserves the manifest notices; query/result validation is pure and imports no corpus I/O or model runtime. Slice C wires these APIs into the internal service; no retrieval HTTP route or UI control is exposed. The browser-free retrieval-contract test above covers them without Ollama, model files or a tokenizer installation.

Internal slice-B APIs are [createExactRetrieval](src/server/retrieval/exact-retrieval.ts) and its [bounded Ollama session](src/server/retrieval/ollama-embedding.ts), [input-fit guard](src/server/retrieval/embedding-input-fit.ts), and [exact ranking](src/server/retrieval/retrieval-ranking.ts). Imports and factory construction perform no model or corpus I/O; only an explicit retrieval call uses the fixed local Ollama boundary. The engine builds all 16 document vectors lazily, reuses only a compatible verified collection, and returns at most three canonical passages. Deterministic embedding-retrieval tests use fake transport and supplied vectors, with the actual in-memory search library; they do not establish real model compatibility or capacity. Slice C integrates the engine into the internal service. The separate [real retrieval-capacity observation](docs/LOCAL_MVP_FEASIBILITY.md#m2-02-retrieval-only-observation--2026-09-08) passed for the recorded model/runtime configuration; UI consumption remains later work.

Internal slice-C APIs are [LocalService.retrieveFinding](src/server/local-service/contracts.ts) and [RunRepository.updateRetrieval](src/server/persistence/run-repository/contracts.ts). Retrieval accepts exactly one `{runId, findingId}` selection, reserves the existing shared service operation, saves the selected running state before execution, and publishes only the selected result or bounded failure. It returns the actual durable completed aggregate without changing native evidence or siblings. The default engine stays lazy; startup, scans and reads do not load the corpus or contact a model. Successful retrieval retains the active selected owner for later workflow stages. There is no retrieval HTTP route or UI control in this task's internal service slice.

### Inspecting M2-02 retrieval evidence

Follow [Run the local service](#run-the-local-service) to open the existing Analyze/Results UI. It displays grouped scanner findings and native rule evidence, not M2-02's retrieved guidance. It has no retrieval action, retrieval-result display, or saved-run reopen/import control. Opening the UI does not start model work. Selected-Finding guidance, support states and abstention presentation belong to [M2-03](docs/DEVELOPMENT_ROADMAP.md#m2-03--apply-support-states-abstention-and-finding-detail-presentation); integrated retrieval and abstention verification belongs to [M2-04](docs/DEVELOPMENT_ROADMAP.md#m2-04--integrate-and-verify-retrieval-and-abstention).
