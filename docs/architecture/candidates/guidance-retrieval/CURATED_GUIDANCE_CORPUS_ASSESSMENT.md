...
Each retrieval run starts from one selected finding and resolves exactly one accepted rule mapping. Every passage carries its rule and success-criterion tags. Retrieval filters on those tags only; it does not preselect source IDs or guidance roles before similarity ranking. The table defines the tagged eligible passages and the roles that the returned passages must collectively support:
...
4. Every passage has a stable `passageId`, corpus version, source title and type, exact heading, direct URL, rule/success-criterion tags, selected text, and a visible normative-or-informative authority label.
