...
4. Resolve one selected Step 2 finding, its per-finding source evidence, normalized projection, rule manifest/mapping, and eligible page-scan reference. Construct one exact privacy-safe query from allowlisted codes; include no URL, locator, HTML, arbitrary page text, input value, credential, or unrelated finding.
...
6. Query the local Chroma evaluation index with exact corpus/rule/SC filters, cosine distance, and fixed `top-k=5`. Do not rerank.
...
LangChain may adapt canonical passages to framework documents, invoke the application-owned embedding/vector adapters, and return ranked passage references. Application code remains authoritative for evidence, query construction, corpus identity, filters, ranking metadata, support-state policy, citations, persistence, and per-finding isolation.
...
| [Local retrieval execution and evaluation](LOCAL_RETRIEVAL_EXECUTION_AND_EVALUATION_ASSESSMENT.md) | Embeddings, vector storage, finding-to-query projection, filters, ranking, citations, support records, alternatives, and model-free evaluation. |
...
7. EmbeddingGemma, Chroma, and thin LangChain remain replaceable evaluation baselines with no reranker, crawler, hosted retrieval, or agent; and
