...
| Fixture loading | Three logical fixture families, each with one failing and one corrected bundled static HTML revision, loaded with Playwright page content; any image bytes are embedded and no network resource is required | **Proposed.** One operation loads only the selected revision. This is sufficient for controlled static pages and is not evidence for real navigation or live-page safety. |
...
- Native incomplete results remain distinguishable from violations, passes, and scan-operation failures.
