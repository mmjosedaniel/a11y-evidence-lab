...
**Status:** Proposed assessment as of 2026-08-24. This assessment elaborates the accepted human-decision boundary in REQ-REV-001 and proposes the smallest review behavior for one reviewer and one proposal at a time. It does not change the status of REQ-REV-002 through REQ-REV-007, accept an open decision, select a UI component or workflow library, authorize development, or claim that the behavior exists.
...
Use one linear, evidence-first proposal review screen backed by the authoritative local application service. The screen presents one proposal version whose content and provenance remain immutable, its exact upstream support, its review readiness, and three explicit reviewer choices: approve it unchanged, edit and accept a reviewer-authored successor, or reject it. The same local reviewer is the MVP authority for deciding whether each material claim and remediation step is semantically supported within the displayed evidence and cited guidance. This human judgment is required before acceptance and is not delegated to another model, reviewer queue, or workflow. This is enough to complete one controlled-fixture MVP review without user accounts, login, reviewer identity authentication, assignment, queues, collaboration, or production workflow orchestration. The accepted authenticated-loopback-API boundary still applies to application requests.
...
1. **Context and status.** Show the sanitized target or fixture label, scan time and declared page state, material scan configuration, supported scenario coverage, proposal version, current lifecycle state, and the label **AI-generated proposal — pending review**. At the point of decision, state that acceptance creates only a remediation plan for later work; it does not modify code, prove that a barrier is fixed, establish whole-page accessibility, establish legal compliance, or provide certification.
2. **Review readiness.** Show evidence sufficiency as `high`, `medium`, or `low`, with the applicable definition, supporting reasons, missing or conflicting facts, unresolved assumptions, citation-resolution status, and blocking manual-check state. The label is a categorical assessment of the available support, not model probability, retrieval score, accessibility score, or compliance likelihood. The MVP should not introduce a separate model-self-confidence meter; uncertainty is communicated through evidence sufficiency, assumptions, limitations, and required checks.
...
A proposal may show `high`, `medium`, or `low` evidence sufficiency, but the label alone does not approve or reject it. Concrete semantic-support confirmations, assumptions, and manual-check conditions control readiness. For an unchanged approval, the reviewer confirms the displayed category under the same rubric used by generation. For an edited successor, the reviewer assesses a successor-specific category under that rubric; the service validates the allowed value, required reasons, affected-claim mapping, and mechanical rubric preconditions. It must not copy the original label automatically or accept an upgrade that lacks explicit existing evidence, resolved assumptions, and reviewer support confirmation. Because edit-and-accept cannot add evidence or guidance in this MVP, a missing basis requires rejection rather than an unsupported upgrade.
...
| `not applicable` | Unset; no outcome was observed | Unset; do not fabricate `supports` or `inconclusive` | Required | Rationale explaining why the check does not apply is required | Clears a blocking check only when the reviewer records that rationale. |

REQ-REV-006 defines no exception path for the first slice: contradictory or inconclusive blocking results prevent acceptance. Acceptance never relabels a pending check as executed, and a later result never rewrites the earlier proposal decision. Rejection remains available when support is insufficient, a blocking check is incomplete, or a contradiction is present. It rejects only the proposal version; it does not reject or alter the deterministic finding, captured evidence, or canonical guidance.
...
4. The evidence-sufficiency category passes the shared rubric and mechanical invariants; its reasons, limitations, assumptions, source authority, and required checks are visible at confirmation time.
...
6. The reviewer explicitly confirms the action, and the proposal transition, review action, accepted-plan pointer where applicable, and any successor version can be recorded together. A persistence failure leaves the proposal pending and offers a safe retry after reading the durable state.

| Reviewer choice | Minimum user-facing behavior | Durable meaning and additional criteria |
| --- | --- | --- |
| **Approve** | Use an unambiguous label such as **Accept proposal as remediation plan**, followed by a summary of the exact proposal version, semantic-support confirmations, evidence sufficiency, and limitations. An optional bounded note may be recorded. | Apply the shared gates to the unchanged AI proposal. Record an `approve` review action and make that exact version the accepted remediation plan. Approval does not approve the finding, page, implementation, accessibility, compliance, certification, or model accuracy. |
| **Edit and accept** | Editing opens a transient working copy; it is not a separate durable action. The reviewer may change only AI interpretation, remediation, assumptions, and the successor's evidence-sufficiency assessment. Generated check definitions and blocking classifications remain read-only. The reviewer may add only a separately labeled, non-blocking post-change verification definition. The final action is **Accept edited remediation plan**; cancel leaves the original pending. | Keep findings, evidence, canonical passages, original generation output, original sufficiency assessment, generated check definitions, and recorded result occurrences immutable. Record an edit reason, the complete reviewer-authored successor, changed-field summary or readable difference, and a link to the original. For every changed material claim or step, the same MVP reviewer must explicitly confirm support from the exact existing evidence and passages. The reviewer assesses successor evidence sufficiency under the same rubric, and the service mechanically validates its value, reasons, affected-claim mapping, and any claimed upgrade. If the edit needs new evidence, guidance, or a newly blocking pre-acceptance check, or if a generated blocking definition is materially wrong and cannot truthfully be marked not applicable, reject with the reason `insufficient evidence or context/manual-check support`; do not accept the edit or invent another workflow action. On success, record the `edit_and_accept` action, mark the AI version superseded, and make the complete human-authored successor the accepted plan together. |
...
The exact editable-field and reason-code vocabulary remains Proposed. Free-form feedback should supplement a small structured taxonomy rather than replace it, because bounded categories are easier to inspect, minimize, and evaluate later. For this candidate, “every human edit” in REQ-REV-002 means the complete reviewer-authored successor submitted through each successful `edit_and_accept` decision. That full version, its reason, readable difference, support confirmations, sufficiency assessment, action, and link to the immutable source version are retained and never overwritten. Transient keystrokes, canceled working copies, and mechanically invalid form attempts are not submitted edits; the MVP has no separate save-draft or submit-edit-without-decision action.

## Proposal lifecycle and allowed transitions

Keep proposal lifecycle, review readiness, manual-check execution, evidence sufficiency, and generation outcome as separate dimensions. This prevents combinations such as `low-confidence-approved-with-checks` from becoming opaque status values.
...
| `pending_review` | Edit and explicitly accept after all gates pass | In one durable operation, change the AI version to `superseded`, create the complete reviewer-authored successor in `accepted`, and append the `edit_and_accept` action linking both versions. |
| `pending_review` | Reject with a reason | Change the same version to `rejected` and append the `reject` action. |
| `pending_review` | Cancel editing, fail validation, encounter a stale revision, or fail before durable commit | Remain `pending_review`; preserve the working view when safe or reconstruct it from durable records, and explain the failure. |
...
Do not introduce a page-specific review entity. The minimum record is a documented composition of the existing **Proposal version**, **Review action**, **Manual-check result**, and referenced upstream records in the [information and workflow lifecycle model](../../requirements/INFORMATION_AND_WORKFLOW_LIFECYCLE.md). The proposal carries immutable manual-check definitions; each **Manual-check result** is a separate execution occurrence and may later reference the exact scan and comparison it verifies. Exact serialized schemas remain under OD-015.

| Record part | Minimum conceptual information |
| --- | --- |
| Identity and lineage | Review-action ID and schema version; exact proposal version; prior and resulting state; and references to the finding, scan, evidence, retrieval run, corpus snapshot, passages, and generation run. Their existing records preserve deeper configuration provenance. |
| Presented decision basis | The original proposal reference; evidence-sufficiency category, rubric version, and reasons; material claim and remediation-step links to evidence and passages; assumptions and limitations; and immutable manual-check definitions with separate result-occurrence references. Prefer references over duplicating sensitive payloads. |
| Decision | Action `approve`, `edit_and_accept`, or `reject`; a stable non-identifying application-local reviewer reference and display label; application-recorded UTC timestamp; reviewer semantic-support disposition for every material claim and remediation step; resulting state; reason category where required; optional bounded sanitized note; and the limitation version shown at confirmation. The actor reference is attribution, not authenticated identity or proof of reviewer qualification. |
| Gate snapshot | Reference-resolution and mechanical-validation results; reviewer semantic-support confirmations; evidence-sufficiency rubric result; required blocking-definition IDs; exact result-occurrence IDs, execution statuses and relationships; and gate outcome at decision time. Later check results must not retroactively rewrite the basis of a prior decision. |
...
The local service assigns the decision time, verifies that the displayed proposal version is still pending, and records the state change, review action, accepted-plan reference, and any edited successor together. After an ambiguous failure or reload, the screen reads that durable state before offering another decision. This is the minimum integrity behavior for one local reviewer, not a workflow-engine or storage selection.
...
The preserved action, structured reason, edited fields, manual-check results, and exact evidence/retrieval/generation identities may help classify later errors and select candidate evaluation cases. Promotion requires a separate, explicitly authorized, versioned offline curation step under OD-009 and OD-017, with sensitivity review, redaction, exact lineage, the approved reviewer rubric, independent qualified review, and adjudication. The product review action must not automatically:
...
Use a stable opaque application-local reviewer reference and a neutral label such as `Local reviewer` rather than an operating-system username or email. Bound and sanitize notes, warn against entering secrets, and keep review history under the retention, protected-storage, cascading-deletion, tombstone, backup, and export policies still to be accepted under OD-006 and OD-012. The review page must not add analytics, cloud tracing, telemetry, automatic source previews, or unrelated egress.
...
- Keyboard and the accepted screen-reader scenario can inspect all layers and citations, understand unavailable actions, edit permitted fields, confirm a decision, recover from errors, and receive state announcements.
...
- The reviewer is competent to judge the selected narrow scenario and is the MVP semantic-support authority before acceptance; the exact primary-user and reviewer relationship remains under OD-001.
...
- Durable local persistence, retention, and schema authorities will be selected before development under OD-006, OD-012, and OD-015.

## Open questions

- Which primary persona performs the product decision, and when is a separate accessibility specialist required under OD-001?
- What exact evidence-sufficiency rubric, reviewer calibration, and low-sufficiency acceptance policy will OD-007 approve?
- Which compact interaction best captures material-claim support confirmation and the edited successor's same-rubric evidence-sufficiency assessment on one screen?
...
- What exact retention, cascading-deletion, note, difference, export, backup, and tombstone rules will OD-006 and OD-012 accept?
- What canonical record/schema and migration authority will OD-015 accept, what persistence and atomic-transaction behavior will OD-012 accept, and how will this module's accepted lifecycle semantics satisfy the canonical-authority requirement in REQ-QUAL-018?
- What reviewer rubric, qualification, evaluation-feedback promotion, and adjudication process will OD-009 and OD-017 accept?
...
- A reviewer may overstate an edited successor's evidence sufficiency even though the edit cannot add evidence or guidance.
