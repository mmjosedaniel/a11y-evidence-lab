...
- Record exact per-rule coverage and a full collection disposition independently of the violation and `ScannerReviewObservation` counts. A navigation timeout, browser/scanner failure, malformed or truncated result, or missing rule coverage cannot become a complete result. Zero Findings is valid only after all required native result arrays and exact coverage for the three trusted-page rules validate.
