...
### Native-schema Local admission amendment — 2026-09-23

**Accepted by the owner on 2026-09-23.** Native schema enforcement with reasoning is selected for the new ordinary Local profile. This prospectively amends the complete-fit obligation above and in REQ-LLM-008 for that profile only. Historical Local profiles and the Groq amendment retain their original meanings.

Prompt-only constraints have not prevented invalid references and reminder wording in the fixed Local cases. The selected option uses the existing runtime's schema mechanism and a fixed neutral reminder. A separate application-managed reasoning/final-answer sequence would add another request and coordination. Native enforcement instead preserves one application request, but the pinned runtime can execute two completions and form a second prompt containing generated reasoning. Accepted source analysis establishes that generated-token counts cannot be assumed to bound that re-tokenized prompt. The owner accepts bounded runtime rejection or exhaustion instead of advance second-prompt fit assurance.

The new `m602-ollama-native-schema-v1` adapter uses prompt `m602-native-schema-instructions-v1`, schema `m602-native-schema-v1`, configuration `m602-qwen35-native-schema-32768-v1` and accounting `m602-qwen35-initial-prompt-bound-v1`. It retains Ollama v0.33.3 and the admitted `qwen3.5:4b` artifact, complete selected evidence, two messages and the complete case schema in the system message. The same schema is transmitted as native `format`. Exact reference and blocking-judgment constraints remain; the fresh-profile reminder is exactly "After changes, rescan and perform relevant human verification." Application validation checks that reminder before and after normalization without changing historical proposal reading or human editing.

Initial admission uses `UTF8(systemIncludingSchema) + UTF8(user) + 88 + 12288 + 32 <= 32768`. The distinct `initial-prompt-upper-bound` accounting identifies its limited guarantee. The separately transmitted grammar is not another prompt copy. Mixed identities, malformed input, changed controls and an excessive initial bound fail before generation. Complete admitted input is preserved; native input truncation and context shifting remain disabled.

The fixed controls remain reasoning enabled, non-streaming, context 32768, `num_predict:12288`, temperature 1, top-p 0.95 and keep-alive five minutes, with the authenticated inherited defaults. The one application chat can contain at most two native completions, each capped at 12288 generated tokens, for an aggregate ceiling of 24576. One absolute 300000-ms operation deadline covers preparation and both native passes; it is never reset between passes. These limits are ceilings, not promised allocation, successful output or an OS process-kill guarantee.

An internal second-prompt rejection or context exhaustion is an attempted bounded failure. Full final-envelope and application validation, thinking disposal, the 1-MiB response bound, metadata deadlines, cleanup controls, no late success, no automatic retry and no fallback remain. HTTP-resource cleanup does not prove native runner cancellation. Native grammar constrains structure but does not replace reference, nonblank, prohibited-claim or human semantic-support checks.

The [M6-02 implementation contract](../../plans/completed/m6-02-six-fixed-generation-executions.md#m602-native-contract--frozen-implementation-semantics-for-review) owns bounded execution and verification. The [forward evaluation amendment](../../requirements/evaluation-and-release/EVALUATION_AND_ACCEPTANCE.md#native-schema-local-amendment--2026-09-23) requires distinct affected evidence and preserves previous observations. This decision neither qualifies capacity nor authorizes live calls by itself. Source basis: [Ollama v0.33.3 native transition](https://github.com/ollama/ollama/blob/v0.33.3/server/routes.go#L2587), [completion controls](https://github.com/ollama/ollama/blob/v0.33.3/llm/llama_server.go#L1441), [Qwen35 rendering](https://github.com/ollama/ollama/blob/v0.33.3/model/renderers/qwen35.go#L264), and [pinned schema-converter limitations](https://github.com/ggml-org/llama.cpp/blob/b10760/common/json-schema-to-grammar.cpp#L358).
