...
ADR-0017 also moved mode selection to one immutable global `Local` or `Groq` context on the `PageAnalysisRun`. Selecting Groq does not send data or automatically process its findings. Each nested selected-Finding workflow was defined as user-selected, independently gated, disclosed before remote invocation, and limited to at most one Groq call. ADR-0020 replaced the separate provider probe and originally moved disclosure to a run-level label; OD-027 later moves that disclosure out of provider-independent Analyze/Results and back to the later provider-relevant workflow without adding another confirmation gate. ADR-0021 replaces the child-record representation with nested data in `run.json`. Explicit per-finding generation, independent gating, disclosure before relevant provider use, and the one-call limit remain current. There is no all-findings batch, automatic retry, or fallback.
...
[ADR-0020](ADR-0020-manual-developer-managed-local-model-setup.md) preserves Groq, `openai/gpt-oss-20b`, minimized egress, explicit generation, and no fallback while simplifying their interface obligations. Its initial placement put one concise disclosure at global mode selection and kept provider/model context visible; OD-027 supersedes that placement as described below. It also performs no synthetic connection or capability probe. The explicitly requested real call and application-owned response validation establish the provider outcome.
...
- When the later selected-Finding workflow makes generation relevant, present concise disclosure identifying Groq, the exact model, remote processing, and the minimized selected-finding evidence and required guidance categories that an explicit Generate action may send. Keep the provider/model context visible in that provider-relevant workspace and its invocation, failure, provenance, proposal, and review states. The ready Analyze form and deterministic scan Results omit it. A repeated confirmation gate is not required, and global Groq mode remains context rather than standing permission for an automatic call.
...
- API mode intentionally introduces network availability, egress, credential, provider-retention, cost, model-lifecycle, and rate-limit constraints. The later provider-relevant workflow and canonical run record must make the selected mode and any provider failure explicit; the ready Analyze form and provider-independent scan Results omit them through OD-027.
