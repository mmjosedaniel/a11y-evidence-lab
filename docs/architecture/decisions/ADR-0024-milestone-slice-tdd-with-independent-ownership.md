...
- Each role receives at most one same-contract correction using attempt 2. A changed contract, repeated decisive failure, exhausted budget, or unresolved authority conflict stops automatic continuation.
...
- The optional hook metrics remain local, best effort, fail open, and non-authoritative. Their absence cannot block implementation or closure.
