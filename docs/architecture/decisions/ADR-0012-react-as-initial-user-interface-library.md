...
- Evaluate the application's core path against `REQ-A11Y-*` using the automated checks and manual visual review governed by `REQ-A11Y-006` and its 2026-09-09 amendment. A formal assistive-technology support matrix remains deferred; browser evidence is required for visual claims that component tests cannot establish.
