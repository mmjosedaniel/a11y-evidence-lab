...
- Generate canonical passage and query vectors locally with the EmbeddingGemma configuration accepted in ADR-0006.
- Build the small vector collection in process from the accepted canonical corpus when the application starts or retrieval is first needed. Do not persist the vector collection or run a vector-database service.
...
- Filter only by the active corpus identity, language, exact supported rule family, and mapped WCAG success criterion. Do not preselect exact source IDs or guidance roles before similarity ranking.
- Rank the eligible passages by exact cosine similarity and return fixed `top-k=3`, using passage ID as the deterministic tie-break for equal scores.
...
- Record the corpus identity, EmbeddingGemma model identity, query representation, applied broad filter, cosine metric, `top-k=3`, ranked passage IDs, and scores needed by the accepted retrieval requirements. Do not create vector-service, collection, persistence-path, HNSW, backup, migration, or deletion records.
...
This decision supersedes ADR-0007 for the MVP. Chroma may be reconsidered only after a larger or mutable corpus, a measured startup/search problem, or an accepted persistence requirement demonstrates that the in-process exact search is insufficient.
...
- The vector collection is rebuilt after process restart. That cost is acceptable for the fixed small corpus and must be measured only as part of the existing compact retrieval evaluation.
