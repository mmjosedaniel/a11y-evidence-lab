...
[ADR-0018](ADR-0018-trusted-operator-url-boundary.md) supersedes ADR-0017 and this record's earlier synthetic-only input statement. The localhost UI may submit one operator-entered public HTTPS URL per `PageAnalysisRun`, but it remains an unprivileged control surface. Authorization and trust are supported-use assumptions rather than a separate confirmation workflow; OD-027 removes the earlier visible M1-04 limitation notice while retaining the documented and admitted trusted-input boundary. The local application service owns the fresh Playwright-managed Chromium context, scanning, evidence, persistence, and provider calls. Authenticated targets, imported browser state, interaction, crawling, and distributed hosting remain excluded.
...
- The UI may accept one trusted operator-entered public HTTPS URL and must enforce ADR-0018's boundary without requiring a separate attestation control. Through OD-027, the limitation is owned by admission and documentation rather than repeated as ready-form or Results copy. The scan path does not connect to an existing external Chrome or Edge session, accept authenticated state, crawl, intentionally interact with the target, download files, or treat redirects or links as additional analysis targets.
