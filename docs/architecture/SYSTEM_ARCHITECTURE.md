...
| --- | --- |
| Browser interface | [App.tsx](../../src/client/App.tsx) composes the interaction and coordinates shared run state and operation reservations. [Components](../../src/client/components) display findings, guidance, proposals, human decisions and comparisons. The [client responsibility map](../DEVELOPMENT.md#client-code) identifies request handling, response admission, form validation and results focus ownership. The interface does not read local files or hold provider credentials. |
