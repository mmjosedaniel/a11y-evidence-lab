...
| --- | --- | --- |
| Authorized deterministic scan slice | One TypeScript module loads the named project-owned fixture revision as static page content in a fresh pinned Chromium context, blocks unexpected requests, runs only axe `image-alt`, and returns a validated transient native observation in memory. | Step 1 owns execution and the transient observation. [Step 2](candidates/ACCESSIBILITY_FINDING_EVIDENCE_CAPTURE_ASSESSMENT.md) exclusively owns allowlisting, sanitization, normalization, digests, and durable evidence. Live pages remain out of scope. |
...
| Sequential operation ownership | One current operation at a time produces a complete result before the next stage consumes it. | No queue, replacement controller, checkpoint engine, or user-cancellation framework is required for the controlled portfolio slice. |
| Portfolio evaluation | One frozen 5–10-case synthetic set and one reviewer rubric evaluate retrieval, grounded generation, abstention, review, and the three definitive comparison demonstrations. | The run is non-promotable and supports no generalized quality, provider-comparison, or release claim. |
...
| Minimal local persistence | Persist only the small record graph needed to reopen the one demonstration, with explicit version fields and safe failure behavior. | OD-006 and OD-012 must select the actual store and retention policy before implementation. Browser Web Storage is not the durable authority. |
