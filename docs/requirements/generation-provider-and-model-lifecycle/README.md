...
| --- | --- | --- |
| [Generation provider execution](GENERATION_PROVIDER_EXECUTION.md) | `REQ-LLM-001` through `REQ-LLM-020` | Provider-neutral Local/Groq execution, one run-level mode and disclosure, per-finding calls, role-specific Local data flow, attempt-time validation, structured output, no fallback, and deferred provider expansion or qualification. |
