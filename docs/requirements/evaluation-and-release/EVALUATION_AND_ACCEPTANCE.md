...
| Shared review | For validated proposals only, exercise `approve`, `edit and accept`, and `reject` while preserving the original proposal, the blocking pre-acceptance judgment and its approval gate, the non-blocking post-change verification reminder, and decision timestamp. |
...
| Human review | For a validated proposal only, one reviewer can approve, edit and accept, or reject while the original proposal, blocking judgment, post-change reminder, action, feedback or edit, and timestamp remain distinguishable. Approval or edit-and-accept requires the blocking judgment to support the resulting proposal or be explicitly not applicable with a reason. |
...
| BHV-05 | Require an individual approve, edit-and-accept, or reject decision before one proposal becomes an accepted plan; gate acceptance on its pre-acceptance judgment while retaining its later verification reminder as non-blocking guidance. | `REQ-REV-001`, `REQ-REV-008`, `REQ-REV-009` |
