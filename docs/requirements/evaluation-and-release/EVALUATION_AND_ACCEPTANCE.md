...
### Controlled generation input exception

**Accepted by the owner on 2026-09-09 for the six fixed generation executions only.** Those executions may use three independently assembled canonical, support-complete input packages, one per controlled profile and identical across Local and Groq. The packages demonstrate generation/provider behavior from controlled guidance. They do not demonstrate that actual embedding and ranking returned the supplied guidance or complete the real end-to-end retrieval path.

This narrowly replaces the completed-actual-retrieval prerequisite for these evaluation inputs under REQ-EVAL-001/007/008 and REQ-GEN-003. It does not change production retrieval or eligibility, the corpus, ranking, required guidance roles, evidence sufficiency, canonical citation validation, privacy, the common output/review contract, no-call failure semantics, or the exactly-six count. Every controlled package still requires complete selected-Finding evidence and all required canonical guidance roles without unresolved conflict. Omit similarity scores and embedding/ranking execution provenance from independently assembled packages; never manufacture a runtime RetrievalResult or relabel an actual abstention as supported. Keep the actual M2-04 retrieval observations unchanged and separately reported.

This is a permanent bounded evaluation-input distinction, not a temporary substitute in the production path. The [roadmap's real-integration rule](../../DEVELOPMENT_ROADMAP.md#temporary-substitutes-and-real-integration-rule) and all later real integration/capacity obligations still apply. The exception creates no application path, authorizes no provider call by itself, and does not extend to later tasks' separate integration demonstrations. Freeze exact controlled provenance, input bytes and interpretation in M3-01 before any applicable model output is inspected. The shared deterministic incomplete-guidance/no-call check remains separate and provider-independent.

### Groq admission amendment to the frozen generation definition

**Accepted by the owner on 2026-09-11, before any of the six fixed model outputs were inspected.** Current M3-03 capacity and M3-05/M6-02 records plus the frozen definition confirm the pre-output state; no model call occurred during this amendment. This applies amended REQ-LLM-008 and [ADR-0020](../../architecture/decisions/ADR-0020-manual-developer-managed-local-model-setup.md#groq-admission-amendment--2026-09-11) through the [M3-04 accepted policy](../../plans/completed/m3-04-groq-adapter.md#m304-fit-amendment-01--proposed-groq-request-admission).

The original `evaluation/m301-generation-v1.json`, SHA-256 `63770583a97e1d0517337474dadc706c1047d78e026e22c7a745ce7674ed9e6b`, and all thirteen referenced files remain byte-for-byte unchanged. For future Groq evaluation, this dated amendment takes precedence over only the following original fit phrases; all unaffected manifest fields continue to govern.

| Original manifest field | Accepted Groq-only forward meaning |
| --- | --- |
...
Before any affected output is inspected, the task/evaluation configuration must bind the original manifest identity, this accepted amendment and fixed policy identity `m304-groq-request-bytes-v1` together. They do not become new durable invocation fields. The policy admits a complete serialized body of at most 65536 UTF-8 bytes and requests the unchanged 4096-token completion ceiling; it neither measures tokens nor guarantees hosted fit or complete input consumption. The owner accepts possible provider rejection and unobservable hosted transformation/truncation.

Original file bytes, controlled case definitions, Local obligations, model/schema/generation controls, exactly six executions, no-call abstention/prerequisite behavior, and failure-as-failed-case treatment remain unchanged. Groq's fit obligation and possible rejection timing change explicitly as above. No actual request, retry, cap adjustment or input rewrite is authorized here. Subsequent material changes after output inspection require the existing successor-manifest/new-evidence route under REQ-EVAL-005/007; no earlier observation may be relabeled retroactively.

### Retrieval selection amendment — 2026-09-12

The owner accepted [ADR-0019's selection amendment](../../architecture/decisions/ADR-0019-in-process-exact-vector-search.md#selection-amendment--2026-09-12) during M3-05. Future actual retrieval evidence must bind the new selection policy and current source/configuration identity. Under REQ-EVAL-005, M3-05 will observe the three existing controlled retrieval profiles once each with their original gold mappings, separately from the two public integration runs. Role coverage obtained by construction is not relevance evidence; retain the ordered passages, original scores, gold matches and support outcome separately, including an unsuitable selected role passage even when another passage matches gold.

The original M2-01 gold, M3-01 generation manifest and its thirteen referenced files remain unchanged. Earlier M2-04 and M3-05 global-three observations retain their original meaning. This forward retrieval change supplies no generation-evaluation result and adds none of the six fixed generation executions; the controlled generation-input exception and Groq amendment remain applicable. New public observations are integration evidence, never evaluation gold. The finite callers and failure interpretation are frozen in the [owning M3-05 plan](../../plans/completed/m3-05-generation-checkpoint.md#m305-d-01--approved-selection-implementation-packet) before execution.

### Native-schema Local amendment — 2026-09-23

The owner accepted [ADR-0020's native-schema Local amendment](../../architecture/decisions/ADR-0020-manual-developer-managed-local-model-setup.md#native-schema-local-admission-amendment--2026-09-23) after the recorded uncertainty-profile outputs. This is a prospective material profile change under REQ-EVAL-005/007. Every existing manifest, request, result and assessment retains its original identity and interpretation; no earlier failed case becomes successful through this amendment.

For the new `m602-native-schema-instructions-v1` / `m602-native-schema-v1` / `m602-ollama-native-schema-v1` tuple, Local admission proves the complete initial prompt plus first-completion reserve fits. Accounting `m602-qwen35-initial-prompt-bound-v1` explicitly does not prove fit for the runtime's second reasoning-prefixed prompt. Initial rejection remains a pre-invocation failure; internal context rejection or exhaustion after dispatch is an attempted failed case. One application request may contain two native completions, each capped at 12288 tokens, under one 300000-ms deadline. No truncation, shifting, retry, fallback, partial-answer acceptance or weakening of runtime validation follows.

Before affected model output is inspected, freeze a new manifest binding the original controlled input identities, this amendment, the exact new tuple, case schemas, wire bodies, controls and failure interpretation. The three affected Local cases require new evidence. Existing Groq uncertainty observations can be inherited only after exact input, prompt, schema, wire, model/control and assessment compatibility is established; otherwise affected Groq cases also require new evidence. The six logical case definitions, original corpus, common proposal contract and separate semantic rubric remain unchanged. New native capacity evidence is required for the materially different two-completion configuration; previous single-completion capacity is historical and cannot be relabeled.

Offline verification establishes neither native completion, capacity nor source-supported prose. REQ-EVAL-008 still requires three runtime-valid results per provider. REQ-EVAL-011 remains Deferred; no new all-semantic-pass release gate is introduced. This amendment and the implementation packet create no live execution allowance.

### Controlled profiles

M2-01 supplies a frozen [closed source manifest](../../../corpus/wcag22-mvp-v1/manifest.json), [16 canonical passages](../../../corpus/wcag22-mvp-v1/passages.json) and [three-profile gold mapping](../../../evaluation/m201-corpus-v1.json), with [final verification and freeze evidence](../../plans/completed/m2-01-closed-corpus-snapshot.md#m201-closure-01--final-freeze-and-documentation-impact). The gold maps historical native observations into expected subsets of the implemented M1 evidence contract. It is not a new scanner observation, retrieved result, model output or complete generation package. M3-01's [generation evaluation manifest](../../../evaluation/m301-generation-v1.json) now defines the controlled generation packages, shared output/instruction contract, controls and rubric under the exception above; its [owning plan](../../plans/completed/m3-01-generation-evaluation-package.md) records static verification and task closure. Runtime validation, provider configuration/capacity and the six model executions remain downstream work. Earlier observations are unchanged.
...
| --- | --- | --- |
| BHV-01 | Reject malformed target input, embedded URL credentials, and unsupported schemes before creating a run or navigating; otherwise analyze one developer-trusted page in its current rendered state and publish one complete human-readable list containing every supported-check Finding while keeping items needing manual review and failures distinct. Keep the trusted-input limitation in the operational/documentation boundary without repeating it as ready-state or Results copy. | `REQ-AUTH-007`, `REQ-AUTH-008`, `REQ-SCAN-006`, `REQ-UX-004`, `REQ-UX-005`, `REQ-UX-010`, `REQ-UX-012`, `REQ-SEC-026`, OD-021, OD-027, ADR-0018 |
...
| BHV-03 | Generate one validated proposal only from complete evidence and completed `supported` guidance. Incomplete evidence or completed `incomplete`, `missing`, or `conflicting` retrieval produces a terminal application-authored abstention that explains the blocking information, confirms no provider call, provides manual-investigation guidance, and never enters proposal review. Retrieval execution or passage-integrity errors fail with no support state and are not abstentions. Local input failing its identified profile's admission, or Groq input failing its fixed byte policy, fails before invocation without abstention. Native-schema Local admission proves initial-prompt fit only; internal second-prompt context failure retains attempted-call provenance. Historical Local complete-fit profiles retain their original guarantee. An admitted Groq request can still fail at the provider and retains attempted-call provenance. | `REQ-RETR-004`, `REQ-GEN-001`–`REQ-GEN-005`, `REQ-GEN-009`, `REQ-GEN-010`, `REQ-LLM-008` |
| BHV-04 | Apply one explicit immutable Local-or-Groq provider/model context to the run and invoke one selected Finding's permitted minimized package at a time. Show provider-relevant disclosure when the later workflow makes generation relevant. Missing selected-mode prerequisites fail before any call or ProviderInvocation; attempted calls retain compact non-secret provenance and expose a bounded authentication, quota/rate-limit, network/provider, or response-validation failure category. Never mix or batch providers, add dedicated or in-place retry, or fall back; any later attempt or provider change requires the ordinary Analyze flow and a new independent run. | `REQ-GEN-008`, `REQ-LLM-002`–`REQ-LLM-005`, `REQ-LLM-007`, `REQ-LLM-009`, `REQ-LLM-019`, `REQ-LLM-021`, `REQ-SEC-016`, ADR-0014, ADR-0020, ADR-0023 |
