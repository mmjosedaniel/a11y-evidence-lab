...
1. Freeze the three project-owned scenario profiles and their six failing/corrected fixture revisions, the proposed eight-artifact W3C corpus snapshot, the proposed bounded 9–12-case synthetic evaluation suite, and the directly applicable record semantics.
2. Implement one reusable closed loop from deterministic scan through evidence capture, LangChain-composed retrieval and structured generation, one human review decision, and one rescan comparison; exercise it separately for `image-alt`, `label`, and `color-contrast` rather than introducing multi-finding orchestration.
3. Evaluate that loop on the reference PC with one capacity-qualified local model and record its limitations.
4. Consider live pages, arbitrary URLs, crawling or a crawler implementation, an external API adapter, broader scenario coverage, installer work, and release hardening only after the portfolio slice demonstrates a concrete need for them.
...
| OD-005 | **Accepted for evaluation only.** | Use LangChain as a thin, replaceable two-step RAG integration baseline. Keep canonical records and deterministic stages application-owned; defer LangGraph and keep LangSmith optional. | [ADR-0013](../architecture/decisions/ADR-0013-langchain-as-initial-rag-integration.md) |
...
| OD-003 | Confirm target types, browser, viewport, page-state capture, and authorization mechanism. | For the first slice, accept only the three named fixture families and their six failing/corrected revisions, one pinned Chromium profile, and the minimal fixture attestation. One operation selects one fixture revision and one rule. Decide live-page policy later; no crawler design belongs to this decision now. |
| OD-004 | Approve corpus sources, source hierarchy, snapshot method, and licensing. | Manually freeze the proposed eight-artifact W3C pack supporting SC 1.1.1, SC 4.1.2, and SC 1.4.3; keep axe and ACT rule-mapping metadata outside the embedded guidance corpus. |
| OD-006 | Approve the evidence capture, in-memory boundary, deterministic sanitization, encryption/access control, retention, cascading deletion, tombstone, backup, export, and local data-storage policy. | For the synthetic slice, retain only the scenario-specific Step 2 allowlisted evidence and records needed to reopen the three demonstrations. Decide private-page, backup, deletion, screenshot, and export policy before those capabilities exist. |
...
| OD-008 | Approve finding correlation rules and per-scenario comparison semantics. | Use exact fixture, scan-profile, axe-rule, and stable target identity. Support binary `resolved`, `persistent`, and inverse-pair `regressed` for `image-alt` and `label`; for contrast, use the retained ordered margin so higher/equal/lower still-failing transitions are `improved`/`persistent`/`regressed`, while only a native pass is `resolved`. Allow `inconclusive` or pair-level `not comparable`. |
| OD-009 | Approve the evaluation dataset, case counts, stratification, reviewer rubric, metric vocabulary, severity scale, browser/assistive-technology matrix, screen-reader scenarios, agreement/adjudication method, and quality thresholds. | Freeze one bounded 9–12-case synthetic suite spanning all three profiles and one reviewer rubric for the non-promotable portfolio pilot. Add statistics, multiple reviewers, and assistive-technology matrices only before broader claims. |
...
| OD-012 | Confirm persistence, export, backup, and migration scope. | Persist only the small local record graphs needed to reopen the three controlled demonstrations, one workflow at a time. Defer backup, migration, general export, and external tracker integration. |
