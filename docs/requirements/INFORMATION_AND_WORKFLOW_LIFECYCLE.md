...
| --- | --- |
| Run identity | Top-level format version; immutable `runId` and creation time; normalized requested and observed final public-page identities; immutable global Local-or-Groq mode; application revision; optional `baselineRunId` only for an intentional later verification scan. |
...
| Finding | Stable within-run `findingId`; exact rule/check and native bucket; immutable minimized rule-specific evidence; relevant measured facts; omission or sanitization notes; and current finding-level state. Run and Finding context replace separate scan, evidence-item, and workflow IDs. |
| Selected-finding analysis | Nested only for a Finding the user processes: the privacy-safe query representation; corpus version; ordered stable `passageId` references and metrics; retrieval support result; optional provider invocation; proposal or abstention; one final review decision when reached; and bounded content-safe failure when applicable. It contains no sibling Finding content. |
| Optional provider invocation | Exists only after one local or Groq call is attempted. It records inherited run mode, provider/model/configuration, prompt/output-contract provenance, request time, non-secret outcome, validation result, and safe usage metadata when available. It has no independent ID and stores no credential or raw request/response payload. |
| Proposal or abstention | A proposal retains the immutable generated finding summary, explanation, evidence and `passageId` citations, remediation, confidence/uncertainty, assumptions, required manual judgments, and validation result. An abstention retains the finding, support state, reason, and manual direction and has no provider invocation. |
| Review decision | One current final object for the immutable original proposal: action `approve`, `edit and accept`, or `reject`; decision time; bounded rule-specific manual judgments; and, when applicable, the complete reviewer-edited proposal or rejection reason/note. It has no actor identity, independent review ID, proposal-successor graph, audit event history, reason taxonomy, per-claim confirmation records, or separately versioned manual-check definitions/results. |
| Comparison | On a later run, `baselineRunId`; baseline `findingId`; the relevant current Finding or nested positive observation; comparability and correlation rationale; before/after minimized evidence; outcome when classifiable; rule-specific delta; and limitations. It has no independent comparison ID or file and makes no page-level accessibility claim. |
...
The immutable global generation mode is selected when the PageAnalysisRun begins, but scanner execution does not depend on it. Selecting Local or Groq creates no provider invocation, performs no automatic finding processing, and cannot change the rule set or findings collection.
...
- Evidence or guidance insufficiency produces `abstained`, preserves the Finding, and creates no provider invocation.
- A retrieval execution failure has no guidance-support state and produces `failed` for that Finding.
...
An intentional later verification scan is also a new PageAnalysisRun, but its aggregate records `baselineRunId`. It parses and navigates the same trusted developer-supplied page and reruns the exact provider-independent three-rule scan. Finding-level comparison data is nested in the later aggregate. The rescan does not automatically regenerate a proposal, invoke a provider, change an earlier review decision, or process every Finding.
...
For a comparable pair, correlation uses the exact rule and one retained exact locator string that matches uniquely. A missing, changed, duplicate, or ambiguous locator is `inconclusive`, never a forced match; the MVP adds no fingerprint, weighted descriptor, threshold, or fuzzy matcher. `resolved` requires complete later coverage and a unique same-target native non-failing observation; disappearance from the violation list, target removal, or locator failure is insufficient. Binary matched failures are `persistent`. Exact comparable contrast failures use the retained contrast margin for `improved`, `persistent`, or `regressed`. A later-only unmatched Finding remains visible without being called `regressed`; `new` remains Deferred.

Review decisions and manual judgments may be displayed as contextual data but never alter deterministic comparability or outcome calculation. No comparison proves accessibility, conformance, certification, or remediation causality.

## Retention and deletion boundary

The MVP retains one minimized `run.json` aggregate needed to reopen a PageAnalysisRun. It contains target and scan provenance, complete Finding and ScannerReviewObservation collections, per-finding evidence and downstream data, retrieval passage references, optional invocation provenance, proposal or abstention, one final review decision, comparison data when applicable, and bounded failure information.

Deleting a run means deleting its exact local directory. The MVP adds no per-finding deletion, database cascade, backup, cloud synchronization, telemetry store, analytics store, audit store, or multi-user retention policy. A later aggregate whose `baselineRunId` points to a deleted directory exposes broken lineage and cannot compare; the application preserves no hidden duplicate.
