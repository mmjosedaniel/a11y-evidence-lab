...
**Repository stage:** Development ready through OD-025 on 2026-08-28. Implementation follows the derived [development roadmap](DEVELOPMENT_ROADMAP.md). RD-002 is Complete. RD-003 is Blocked during checkout/clean-start reproducibility corrections: LF checkout verification passed, but acquisition requires network permission and a bounded retry allowance. Its original native observations remain accepted. M1-01 is unselected and blocked on renewed RD-003 closure. No application implementation has started.
...
- [RD-002](plans/completed/rd-002-minimum-development-toolchain-literals.md#accepted-synthesis-and-ordinary-literal-freeze) selects the minimum M1 package versions and local-service host. [RD-003](plans/rd-003-scan-evaluation-boundary.md#accepted-setup-and-native-observations--rd003-observations-001) supplies the frozen scan-only profile and six native fixture observations; application and public-page behavior remain later-task work, with no release qualification.
