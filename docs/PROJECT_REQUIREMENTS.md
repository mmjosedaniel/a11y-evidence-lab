...
The requirements-definition request and the subsequent user directions on provider choice, the historical Windows-delivery direction, existing-PC capacity, and technology evaluation baselines are the acceptance authority for decisions recorded on 2026-08-23. OD-019's 2026-08-24 three-scenario direction superseded OD-002 and now remains the fixed evaluation baseline. On 2026-08-25, explicit product direction accepted the primary frontend-developer and secondary-QA personas; local-browser startup; two generation modes using a downloaded local model or Groq; no automatic fallback; bounded local retention; and the other narrowed MVP behaviors recorded through OD-003 to OD-018. OD-020 subsequently accepted one authorized public HTTPS page per analysis, the exact three-rule variable-finding scan, global immutable generation mode, and one-finding-at-a-time downstream work. On 2026-08-27, OD-021 retained that interaction while replacing the hostile-public-target security model with a trusted developer-input portfolio boundary. The historical decisions remain visible rather than silently rewritten.
...
| `REQ-AUTH-007`, `REQ-AUTH-008`, `REQ-SCAN-006`, `REQ-SCAN-007`, `REQ-EVID-001`, `REQ-EVID-007`–`REQ-EVID-009`, `REQ-RETR-005`, `REQ-GEN-010`, `REQ-REV-008`, and `REQ-UX-010`–`REQ-UX-012` | [OD-020](requirements/DELIVERY_READINESS_AND_OPEN_DECISIONS.md#od-020--authorized-public-page-analysis-scope), [OD-021](requirements/DELIVERY_READINESS_AND_OPEN_DECISIONS.md#od-021--trusted-operator-url-boundary-for-the-portfolio-mvp), [ADR-0018](architecture/decisions/ADR-0018-trusted-operator-url-boundary.md), [Project overview — Planned workflow and boundaries](../README.md), and [Project concept — Possible user flow and intended boundaries](PROJECT_CONCEPT.md) |
...
| `REQ-LLM-001`–`REQ-LLM-010`, `REQ-LLM-015`, `REQ-LLM-016`, `REQ-LLM-018`, and `REQ-SEC-005` | [ADR-0001 — Interchangeable generation providers](architecture/decisions/ADR-0001-interchangeable-generation-providers.md) and [ADR-0014 — Groq as the MVP external provider](architecture/decisions/ADR-0014-groq-as-mvp-external-generation-provider.md): provider-neutrality, one global explicit Local/Groq mode, finding-specific invocation, disclosed minimized egress, and no fallback are binding MVP scope; `openai/gpt-oss-20b` is the fixed evaluation configuration only |
