...
- **Implementation status:** RD-001 through RD-003, M1-01 through M1-05, M2-01 through M2-04, M3-01, M3-02 and M3-04 are Complete. Shared generation and both fixed adapters are implemented with controlled verification. User-facing generation, actual eligible provider execution and Qwen capacity remain unverified.
- **Latest fully closed task:** [M3-04 — Groq adapter](plans/completed/m3-04-groq-adapter.md#m304-final-01--integrated-review-and-documentation-closure). Controlled adapter verification, all 22 authoritative suites, independent strict TypeScript, client build, fresh critical review and documentation closure pass; no actual credential or provider call was used.
- **Current tasks:** [M3-03 — Qwen adapter and capacity screen](plans/m3-03-qwen-adapter-and-capacity-screen.md) remains Blocked on authentic supported live retrieval and the required reviewer interface after accepted G and A/B implementation/reviews. [M3-05 — Generation checkpoint](plans/m3-05-generation-checkpoint.md) is In progress for owner-requested planning only under the accepted start-dependency amendment. Its service/UI integration and real Local/Groq calls remain pending. All other later tasks remain Not started.
...
`RD-001`, `RD-002`, and `RD-003` are Complete. RD-003's checkout and clean-start corrections passed execution, independent reviews, final cleanup, and renewed documentation closure. M1-01 is Complete after implementing and verifying pure runtime contracts. M1-02 is Complete after verified local storage and loopback service implementation. M1-03 and M1-04 are Complete after their task-specific verification and closure gates. M1-05 is Complete again after the two post-closure corrections passed 335 tests, strict TypeScript, fresh independent reviews, exact cleanup, and renewed documentation closure. The earlier public-page smoke remains historical evidence and was not repeated. M2-01 is Complete with the verified closed corpus and three gold mappings. M2-02 is Complete: all three application slices, all 398 tests, strict TypeScript, client build, the real retrieval-capacity gate and different final integrated critical review pass. The cold retrieval completed in 3846.0786 ms with the app and Chrome active; validated durable readback preserves native and nonselected evidence. Exact model/runtime provenance, sampled resources, UI timing limits and qualified post-success driver cleanup remain recorded. Its documentation closure is accepted. M2-03 is Complete again after correcting citation navigation and empty-result corpus-version presentation, passing all 448 tests, strict/build, bounded visual checks, independent re-review and renewed documentation closure. The original actual-service proof remains a prior-client observation with unchanged backend/corpus provenance. M2-04 is Complete after all three actual gold-profile observations, the controlled checkpoint, 449-test regression, strict/build, bounded visual checks, different fresh integrated critical review and documentation closure passed; M3-01 is Complete with its frozen controlled generation definition, M3-02 is Complete after shared-stage verification and documentation closure, M3-03 is Blocked on real capacity prerequisites after its Local adapter and A/B reviews passed, M3-04 is Complete after controlled adapter verification, final critical review and documentation closure, M3-05 is In progress for planning only, and the other eleven application tasks remain Not started. M2-03 implements user-facing retrieval, deterministic support states and abstention; its visual evidence limits and owner-authorized detailed zoom deferral to M6-03 are recorded in the owning plan. The fixed Local and Groq adapters are implemented with controlled verification; actual Qwen capacity and eligible provider execution remain pending.
...
The two labeled implementation-checkpoint edges use the owner-accepted start conditions in M3-05 and M4-01. They do not waive completion verification or authorize starting those tasks.
...
flowchart TD
    ...
    M303 -->|accepted A/B implementation and reviews| M305[M3-05 Generation checkpoint]
    M304 --> M305

    M305 -->|accepted implementation/interface checkpoint| M401[M4-01 Review behavior and persistence]
...
- **User-visible outcome:** Enables correct results, failure, mode, and retained-evidence behavior.
...
- **Parent milestone / role / status:** M1 / service and persistence / **Complete**.
- **Execution and closure (2026-08-30 UTC):** The later exact-task request authorized bounded research, guarded implementation, verification, task-owned cleanup and documentation closure. Fresh clean HEAD `207a620`, pinned runtime and unchanged prerequisites are recorded in the same ExecPlan. Both R3 checkpoints and the pre-write ignore gate passed. Separate guarded storage/service Red and Green passed; the service S3 finding was resolved with one correction per original writer and a complete correction review. All 182 product tests, independent strict typechecking, actual startup/reopen/stop and exact synthetic-run deletion passed. Both slice S3 reviews and the different final integrated critical review passed; documentation closure is accepted. The same ExecPlan is archived with original failures and budgets preserved. Only M1-02 was executed; no commit, push, publication or other task was authorized.
- **Activation and readiness (2026-08-30 UTC):** The owner explicitly requested the M1-02 ExecPlan, selecting only this dependency-ready task for planning. The clean baseline is `a3058fa153e3bc96fce361cd301d0e1e1d7b961c`; M1-01 is Complete, its 58 tests and independent strict typecheck pass under the pinned runtime, and RD-003 inputs remain unchanged. The [M1-02 ExecPlan](plans/completed/m1-02-local-service-and-aggregate.md) records the future R3 persistence/service literal freeze, exact-command gate, pre-write Git-ignore protection, two sequential guarded TDD slices, and critical reviews. No worker write, run-data creation, service execution, other task, commit, or push is authorized by this planning request.
- **Objective:** Provide the minimum application-owned boundary for service startup/shutdown and for creating, durably writing, and validating one local run aggregate.
...
- **User-visible outcome:** Service readiness/failure is understandable; completed evidence remains durable for later workflow work; persistence or busy-operation failure is visible. User-facing reopening is Deferred through OD-026.
...
- **User-visible outcome:** The user can start the scan, identify what needs attention, inspect concrete deterministic evidence, and distinguish automated findings, manual-review items, valid zero, and failure without interpreting run or scanner internals.
- **Authorities:** `REQ-LLM-002`, `REQ-LLM-003`, `REQ-LLM-021`, `REQ-INST-004`, `REQ-SEC-005`, `REQ-UX-002`–`REQ-UX-005`, `REQ-UX-010`–`REQ-UX-014`, `REQ-QUAL-012`, `REQ-A11Y-001`–`REQ-A11Y-004`, `REQ-A11Y-009`, `REQ-A11Y-010`, ADR-0012, SPEC-001, SPEC-008, HS-015, OD-026, and OD-027.
- **Verification:** Neither mode is preselected; invalid submission exposes associated URL and mode errors; valid selection shows no normal provider/model explanation; any known missing-configuration message performs no probe and does not block scanning; status and source distinctions do not rely only on color; overlapping actions are unavailable with an accessible explanation; completion, failure, zero, and selection are predictably announced without moving focus from the selected Finding; every Finding and manual-review observation remains present; external content is inert text; 320-pixel and actual 200% views avoid horizontal page scrolling; limitations prohibit accessibility/compliance interpretations.
- **Likely surfaces:** Target form, provider-mode control, shared status region, Results overview, unified grouped result list, shared evidence detail, and responsive styling.
- **Out of scope:** Retained-run reopening, reload restoration, deep links, recent-run history, retrieval controls, Generate action, proposal/review UI, embedded target preview, chat, dashboard, settings, or bulk operations.

### M1-05 — Integrate and verify the walking skeleton

- **Parent milestone / role / status:** M1 / integration and verification / **Complete**.
- **Readiness:** M1-02, M1-03, and M1-04 are Complete. The owner explicitly selected M1-05 and completed its behavior-preserving local-service, scanner, run-contract, and run-repository structural slices plus the [bounded App dead-state cleanup](plans/completed/m1-05-walking-skeleton-integration.md#m105-app-dead-state-cleanup-05--retain-the-app-coordinator). Applicable Must requirements and decisions remain Accepted or explicitly Deferred, the RD-003 evaluation boundary is unchanged, and the original integration/public-smoke record is preserved. The owner authorized two reproduced P2 corrections at `1f92fe8`; their separate Red/Green, all 335 tests, strict TypeScript, fresh S3 and different final integrated review, exact cleanup, and renewed documentation closure passed. M1-05 is Complete again.
- **Execution plan:** [M1-05 — Walking-skeleton integration](plans/completed/m1-05-walking-skeleton-integration.md); [progress summary](progress/m1-05-walking-skeleton-integration.md).
...
- **Authorities:** `REQ-AUTH-007`, `REQ-AUTH-008`, `REQ-LLM-002`, `REQ-LLM-003`, `REQ-LLM-021`, `REQ-INST-002`, `REQ-INST-004`, `REQ-SCAN-001`, `REQ-SCAN-005`–`REQ-SCAN-007`, `REQ-EVID-003`, `REQ-EVID-007`–`REQ-EVID-011`, `REQ-SEC-005`, `REQ-SEC-006`, `REQ-UX-002`–`REQ-UX-005`, `REQ-UX-010`–`REQ-UX-014`, `REQ-QUAL-011`, `REQ-QUAL-012`, `REQ-QUAL-020`, BHV-01, BHV-07, HS-001, HS-004, HS-006, OD-021, OD-024, OD-026, and OD-027.
- **Verification:** Demonstrate service readiness/failure/clean stop, valid nonzero, valid zero, native incomplete, malformed input, visible scan failure, durable aggregate publication and validated internal reads, the manual exact-directory deletion boundary, one-operation guarding, initially unselected mode and validation-only feedback, known-configuration messaging without a provider probe or scan block, sibling preservation, cleanup, and no provider invocation. Confirm that the MVP browser UI exposes no manual Run ID or retained-run reopen action. Include the applicable automated and keyboard checks rather than postponing accessible behavior.
...
- **Parent milestone / role / status:** M2 / enabling and validation / **Complete**.
- **Activation and readiness (2026-09-03 UTC):** The owner explicitly requested the M2-01 ExecPlan, selecting only this dependency-ready task for planning. M1-05 is Complete; applicable corpus/evaluation requirements and ADR-0022 are Accepted, and RD-003 inputs and the verified M1 Finding contract are unchanged. Fresh strict TypeScript and 78 pure contract/normalization tests passed. That original planning request did not authorize execution. The subsequent 2026-09-03 grant authorizes M2-01 execution and exact-eight acquisition; current readiness and artifact-specific local-use review passed. The first guarded request failed to connect with curl exit 7, captured no file, and was not retried. The lease closed compliantly; all 82 protected inputs remain unchanged. The owner then authorized one outside-sandbox correction; all eight captures and fresh S0 review passed. The 16-passage corpus and three gold cases passed structural, reconstruction, five-negative and manual checks; final review, cleanup and documentation closure passed. M2-02 stays Not started; no commit or push is authorized.
- **Completion evidence (2026-09-03 UTC):** [Final verification and closure](plans/completed/m2-01-closed-corpus-snapshot.md#m201-closure-01--final-freeze-and-documentation-impact) records the accepted eight-source/16-passage snapshot, three gold cases, preserved inputs, reviews and verified cleanup. At that closure nine roadmap tasks were Complete and nineteen remained Not started; subsequent M2-02 planning activation is recorded below.
- **Execution plan:** [M2-01 — Closed corpus snapshot](plans/completed/m2-01-closed-corpus-snapshot.md); [progress summary](progress/m2-01-closed-corpus-snapshot.md).
...
- **Parent milestone / role / status:** M2 / service and integration / **Complete**.
- **Completion evidence (2026-09-08 UTC):** [M202-CLOSURE-01](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-closure-01--final-integrated-verification-and-documentation-impact) accepts L1–L7 and I1–I9: all three application slices, 398 passing tests, strict/build, exact runtime/model and finite-input provenance, the real 3846.0786-ms retrieval and selected-only durable readback, different fresh S3 integrated PASS, owned app/tab closure and documentation gate. UI timing and sparse resources remain bounded observations, driver exit 1 remains a post-success cleanup qualification, and the owner-approved developer runtime plus retained artifacts are not deleted. M2-03 and later tasks stay Not started. Earlier status and pending statements in this task's dated checkpoints are historical.
- **Owner-approved continuation (2026-09-07 UTC):** The owner accepts the documented old-dependency backup limitation and authorizes validation of the existing installation followed by slice A, without repeated downloads. The prior failed preservation claim remains invalid; no original-byte reconstruction or broader recovery is selected. M2-02 resumes its existing scope and readiness; the accepted setup and slice-A results follow.
- **Accepted setup (2026-09-07 UTC):** Exact package pins, the additive lock graph, isolated restore, imports and supplied-vector canary passed. After the owner accepted the documented backup limitation, fresh strict TypeScript and all 78 pure tests passed, followed by fresh S0 review without findings. Both dependency trees and downloads remain retained. The subsequent slice-A checkpoint below owns application evidence; setup itself establishes no model run or application behavior.
- **Accepted slice A (2026-09-07 UTC):** The [M202-A-01 catalog/query/result contract](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-a-01--bounded-implementation-contract) is implemented through independently owned Red/Green and closed-compliant leases. Primary strict TypeScript and 90 focused tests pass; fresh S3 review returns PASS without findings and independently repeats that barrier plus 20 critical probes. Application slices are 1/3 complete, not an overall ExecPlan percentage. B embedding/ranking execution, C durable service integration and actual capacity/I9 remain pending. No model run or task completion is claimed.
- **Accepted slice B (2026-09-08 UTC):** The [M202-B-01 embedding/exact-retrieval contract](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-b-01--bounded-embedding-and-exact-retrieval-implementation-contract) is implemented with fixed local transport, finite input admission, exact canonical ranking and a lazy compatible collection. Four S3 findings were resolved through accepted regression Red and the sole separate Green correction, with compliant closed leases. All 118 focused tests, strict TypeScript and complete corrected-surface S3 review pass; primary accepts the focused composition. Application slices are now 2/3 complete, not an overall ExecPlan percentage. C durable service integration, actual runtime/configuration linkage and capacity/I9 remain pending. M2-02 stays In progress; no model run or later-task selection is claimed.
- **Accepted slice C (2026-09-08 UTC):** The [selected-Finding service/publication contract](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-c-01--bounded-selected-finding-service-and-publication-contract) and [signed-zero durability correction](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-c-durable-01--owner-resumed-json-durability-correction) are accepted after separate test/implementation ownership, compliant closed leases, primary validation and complete C S3 re-review. The C barrier passed 260 distinct bounded tests and strict TypeScript; subsequent M202-INTEGRATION-01 passes the complete 398-test ten-file suite, strict TypeScript and the client build, including all browser and entry groups. Returned success values match durable JSON readback; native scan evidence, siblings, strict transitions and shutdown/commit truth remain protected. Application slices are 3/3 complete, not an overall ExecPlan percentage. Actual runtime/configuration linkage, capacity/I9 and task-level integrated review remain pending. M2-02 stays In progress; no actual model run or later-task selection is claimed.
- **Activation and readiness (2026-09-04 UTC):** The owner explicitly requested the M2-02 ExecPlan after M2-01 completion, selecting only this task for planning. The clean planning baseline is `56a1c97e59311be604b6e96b8e4ee06643f0741c`; applicable requirements and ADRs are Accepted, the corpus/gold and M1 executable inputs remain unchanged, and fresh strict TypeScript plus 78 pure tests pass. The plan reserves the R3 literal/command freeze, metadata prefilter, developer-managed setup, guarded TDD, and real retrieval-capacity evidence. That original request authorized planning only. The subsequent owner request authorizes M2-02 execution subject to its gates. Fresh execution HEAD `d95aa6c8fb26d4ab897cc76d1fbe6515c178d051`, strict TypeScript and 78 pure tests pass with frozen inputs unchanged. Bounded R3 research and mandatory analysis reached [OWNER DIRECTION](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-research-stop-01--owner-direction-at-the-literal-gate): verified pre-call token accounting needs an additional artifact/access route and renewed evidence allowance; unavoidable transitive SDK/LangSmith effects need explicit clarification. Neither literal checkpoint nor setup, implementation or capacity gates passed. No package/model acquisition, model call, other task, commit or push occurred. The owner subsequently approved one additional bounded tokenizer investigation and analyst recheck plus the stated transitive-dependency exception; [M202-REENTRY-01](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-reentry-01--authorized-tokenizer-investigation-and-dependency-exception) records the allowance. The additional report and analyst recheck identified [the pinned seven-file candidate packet](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-artifact-input-01--pinned-tokenizer-inspection-prerequisite). The owner subsequently completed personal consent and authorized an isolated downloader and permanent external destination; [verified receipt](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-artifact-receipt-01--owner-authorized-tokenizer-only-receipt) now proves all seven file sizes and identities. M2-02 remains Blocked pending controlled offline validation of exact tokenizer/runtime equivalence, not input acquisition or dependency permission. The owner then authorized one actual-artifact inspection and analyst recheck; [M202-COMPATIBILITY-01](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-compatibility-01--owner-authorized-actual-artifact-investigation) records their completed, narrowly SYNTHESIS READY answer: structural support is plausible but exact runtime-equivalent counting remains unproved. No literal, implementation or capacity gate is passed, and further offline tooling/metadata effects require a scoped route.
- **Execution plan:** [M2-02 — Embedding retrieval and capacity gate](plans/completed/m2-02-embedding-retrieval-capacity-gate.md); [progress summary](progress/m2-02-embedding-retrieval-capacity-gate.md).
- **Prior enabling checkpoint (2026-09-07 UTC) — historical:** [Finite-input validation](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#m202-finite-validation-01--owner-approved-finite-input-validation) passes after the exact whitespace correction and fresh complete S3 review. Independent readback confirms all 126 ordered native/JavaScript arrays agree over 63 unchanged inputs, zero mismatches, and maximum raw/embedding counts 548/550 within C=2048. Original inputs, source/Git and installed files are preserved and phase processes are closed. This resolves the offline tokenizer comparison, not the remaining L1–L7 literal acceptance, production projection coverage, installed-runtime/configuration linkage or real embedding-capacity evidence. Application slices remain 0/3 and I9 NOT RUN; task status remains Blocked at literal acceptance. The preceding activation/inspection record is historical.
- **Accepted literal checkpoint (2026-09-07 UTC):** The [complete L1–L7 policy contract](plans/completed/m2-02-embedding-retrieval-capacity-gate.md#authored-literals) passes mandatory DRAFT READY synthesis, fresh pre-draft R3 review, different fresh final R3 review and primary reconciliation, with no findings. It freezes the task-owned implementation semantics and later-proof boundaries without a new ADR or requirement change. M2-02 remains Blocked awaiting the separately required pinned dependency-setup approval. Installed effects, production projection/runtime linkage, all three application slices and actual capacity remain pending; I9 is NOT RUN. No application/dependency write, model operation, task-status transition or later-task selection occurred at this checkpoint.
- **Approved implementation continuation (2026-09-07 UTC):** The owner authorizes the exact L1 dependency installation, preservation of the existing dependency tree and downloads, and slice A. Primary rechecks the unchanged 173-file endpoint, completed M2-01 dependency, Accepted task authorities, frozen evaluation/corpus inputs and both literal reviews, then returns only M2-02 to In progress. Setup and behavioral evidence remain pending; this grants no model run or later task.
...
- **Parent milestone / role / status:** M2 / validation, service, and UI / **Complete**.
- **Renewed closure (2026-09-09):** [M203-C-POST01](plans/completed/m2-03-sufficiency-abstention-and-detail-ui.md#m203-c-post01-closure--renewed-task-closure) resolves both citation-presentation findings and the test interception blocker. All 448 tests (439 unchanged plus 9 corrected guidance cases), independent strict/build, desktop/narrow correction-region inspection, clean teardown and fresh independent PASS are accepted; documentation closure passes. Full-detail 200% remains deferred to M6-03 and manual screen-reader testing remains excluded.
- **Reopened (2026-09-09):** The owner approved one test-worker and one UI-worker write turn, verification and independent review for the two citation-presentation findings. The original closure below is historical; the renewed closure above supersedes this reopening checkpoint.
- **Original closure evidence (2026-09-09; superseded by renewed closure above):** [M203-FINAL-01](plans/completed/m2-03-sufficiency-abstention-and-detail-ui.md#m203-final-01--closure-evidence-and-explicit-manual-check-deferral) accepts slices A–C, 448 tests, independent strict/build, the single actual abstained guidance-to-disk proof, fresh final critical PASS WITH FOLLOW-UPS and documentation closure. The consumed proof helper's unused supported branch is explicitly non-reusable pending separate correction. Manual screen-reader testing is excluded by owner direction; full-guidance/detail readability and reflow at actual 200% remain deferred to M6-03.
- **Activation (2026-09-08 UTC):** The owner initially selected M2-03 for current-state review and ExecPlan creation, then explicitly authorized execution, required research, guarded implementation, bounded local integration proof, verification and documentation closure. M2-02, including its embedding-capacity gate, is Complete; applicable requirements and decisions are Accepted and the frozen scanner/corpus inputs are preserved. The archived plan preserves entry, literal, execution and closure evidence.
- **Execution plan:** [M2-03 — Sufficiency, abstention, and detail UI](plans/completed/m2-03-sufficiency-abstention-and-detail-ui.md); [progress summary](progress/m2-03-sufficiency-abstention-and-detail-ui.md).
...
- **Verification exception (2026-09-09):** The owner authorized completion using collected automated, browser and actual-service evidence, deferred detailed visual zoom inspection to M6-03, and then limited manual portfolio verification to visual elements. Automated keyboard/focus/announcement checks remain required. Actual 200% browser-menu/form evidence is recorded; spoken abstention/reselection and full-detail zoom proof are not claimed. Expected accessibility behavior remains unchanged.
...
- **Parent milestone / role / status:** M2 / integration and verification / **Complete**.
- **Current boundary (2026-09-09):** The [bounded checkpoint observations](plans/completed/m2-04-retrieval-checkpoint.md#m204-b-accept-01--bounded-checkpoint-observations) pass: all three actual profiles return frozen acceptable gold targets with exact canonical citations and preserved durable evidence. Each actual set lacks a required guidance role and correctly abstains without generation. Controlled S/A/Z/F/I and the pure conflict boundary pass; the original failed S is preserved separately. All three actual activations are consumed. Final 449-test regression, strict TypeScript, unchanged build, bounded visual checks, different fresh integrated critical review and documentation closure pass. The [final disposition](plans/completed/m2-04-retrieval-checkpoint.md#m204-final-01--integrated-review-and-task-closure) permits proceeding to separately authorized M3-01; no later task is started.
- **Planning activation (2026-09-09, historical):** The owner initially selected current-state review and ExecPlan creation after M2-03's renewed closure and M2-02's passed capacity gate. That request authorized planning only. The later execution activation below authorized the completed checkpoint. The archived [ExecPlan](plans/completed/m2-04-retrieval-checkpoint.md) preserves entry, proof preparation, verification and closure; the [progress summary](progress/m2-04-retrieval-checkpoint.md) mirrors accepted material checkpoints.
- **Execution activation (2026-09-09):** The subsequent owner request authorizes bounded preparation, delegated test work, at most three actual guidance activations under the approved local embedding configuration, verification, corrections within existing budgets and documentation closure. [M204-ENTRY-01](plans/completed/m2-04-retrieval-checkpoint.md#m204-entry-01--authorized-execution-entry) records the clean current baseline and passed entry checks. This entry authorized the checkpoint subsequently accepted above; no generation, configuration change, download, public scan or M3-01 work was selected.
...
- **Parent milestone / role / status:** M3 / validation / **Complete**.
- **Completion evidence (2026-09-09):** The [accepted closure](plans/completed/m3-01-generation-evaluation-package.md#m301-final-01--final-review-and-documentation-closure) records three controlled cases, six common-provider bindings, nine exact local files and the safe [generation manifest](../evaluation/m301-generation-v1.json). Mandatory synthesis, corrected fresh pre-draft PASS, different final acceptance with its documentation follow-up resolved, the 96-negative packet, strict/449-test regression and documentation closure pass. The [owner-approved exception](requirements/evaluation-and-release/EVALUATION_AND_ACCEPTANCE.md#controlled-generation-input-exception) preserves actual retrieval abstentions, production eligibility and later integration/capacity obligations. No model execution or later task occurred.
- **Planning activation (2026-09-09, historical):** The owner initially selected current-state review and ExecPlan creation after M2-04 completion. That authorization was planning only and was superseded by the execution checkpoint above. The [ExecPlan](plans/completed/m3-01-generation-evaluation-package.md) preserves planning readiness and the three real M2-04 abstentions; the [progress record](progress/m3-01-generation-evaluation-package.md) mirrors accepted checkpoints.
...
- **Delivered surfaces:** Static evaluation manifest, exact ignored packages, shared output/instruction contract, controls, rubric and prohibited-claim/failure policy; runtime implementation remains downstream.
...
- **Parent milestone / role / status:** M3 / contract, service, and validation / **Complete**.
- **Completion evidence (2026-09-10):** [Final integrated review and documentation closure](plans/completed/m3-02-shared-generation-stage.md#m302-final-01--final-integrated-review-and-documentation-closure) accepts exact-input recovery, the shared runtime contract, three separately owned implementation slices, complete regression, strict/build and independent reviews. Normal startup has no actual generation adapter; M3-03/M3-04 and the Generate UI remain later work.
- **Execution activation (2026-09-09):** The owner authorized this exact task through prerequisite recovery, remaining runtime decisions, delegated implementation, bounded corrections, verification and documentation closure. [Execution entry](plans/completed/m3-02-shared-generation-stage.md#m302-exec-01--execution-authorization-and-g0-preparation) records original-byte recovery and current verification; the living plan owns subsequent gates and evidence. No actual provider operation or later task is selected.
- **Planning activation (2026-09-09 UTC):** Owner-selected project-state review and [ExecPlan creation](plans/completed/m3-02-shared-generation-stage.md), not implementation. M3-01 remains Complete; current checkout conversion requires exact frozen-manifest recovery before dependent execution. The plan bounds remaining runtime decisions and three separate-owner TDD slices; fresh strict TypeScript and 119 focused tests pass. [Progress](progress/m3-02-shared-generation-stage.md) mirrors accepted planning checkpoints. No provider operation, application change or later task is authorized by this activation.
...
- **Parent milestone / role / status:** M3 / service, integration, and verification / **Blocked**.
- **Execution checkpoint (2026-09-10 UTC):** Developer setup, P and the complete G contract are accepted. [A/B implementation](plans/m3-03-qwen-adapter-and-capacity-screen.md#m303-b-accept-01--accepted-internal-local-integration) passed separate-owner tests, independent strict and fresh critical reviews; the full 550-test suite and client build pass. The fixed Local adapter is implemented. [C remains blocked](plans/m3-03-qwen-adapter-and-capacity-screen.md#m303-c-block-01--exact-remaining-capacity-prerequisites) on authentic supported live retrieval and the implemented reviewer interface. No actual generation occurred. The accepted M3-05/M4-01 start amendments retain every completion requirement and select no later task.
- **Execution activation (2026-09-10 UTC):** The owner authorizes M3-03 through its prescribed gates, delegated implementation, bounded corrections, verification and documentation closure. [Execution entry](plans/m3-03-qwen-adapter-and-capacity-screen.md#m303-entry-02--execution-entry-and-read-only-command-binding) records current Git/lease state, prerequisite/freeze verification and read-only hardware/runtime inventory. This supersedes planning-only authorization without waiving acquisition, permission, license, owner-decision or actual-capacity requirements. No later task is selected.
- **Planning activation (2026-09-10 UTC):** The owner selected current-project review and [ExecPlan creation](plans/m3-03-qwen-adapter-and-capacity-screen.md), not implementation, acquisition or model execution. M3-02 and the separate EmbeddingGemma capacity gate remain Complete. Fresh strict TypeScript, 99 focused tests and the original manifest plus thirteen referenced hashes pass. The plan gates actual accounting, eligible retrieval and representative-interface proof before the single Qwen smoke; [progress](progress/m3-03-qwen-adapter-and-capacity-screen.md) mirrors accepted checkpoints. No prerequisite or verification requirement is narrowed by planning.
...
- **Authorities:** `REQ-LLM-003`, `REQ-LLM-005`, `REQ-LLM-007`, `REQ-LLM-009`, `REQ-LLM-011`, `REQ-LLM-021`, `REQ-INST-005`, `REQ-INST-017`, `REQ-SEC-017`, `REQ-QUAL-006`, `REQ-QUAL-007`, `REQ-QUAL-009`, ADR-0003, ADR-0004, ADR-0005, ADR-0020, ADR-0023, SPEC-004, and HS-009.
...
- **Parent milestone / role / status:** M3 / service and integration / **Complete**.
- **Completion (2026-09-12 UTC):** A0/A/B implementation and critical reviews pass, as do all 22 authoritative suites, independent strict TypeScript, client build and documentation closure. [Verification](plans/completed/m3-04-groq-adapter.md#m304-verification-accept-01--complete-authoritative-regression) maps the task requirements; [final acceptance](plans/completed/m3-04-groq-adapter.md#m304-final-01--integrated-review-and-documentation-closure) preserves evidence limits. The adapter is implemented with controlled verification; no actual credential or provider call was used. M3-03 capacity remains Blocked and M3-05 remains Not started.
- **Accepted fit amendment (2026-09-11 UTC):** The owner accepted the independently reviewed [M304-FIT-AMENDMENT-01](plans/completed/m3-04-groq-adapter.md#m304-fit-amendment-01--proposed-groq-request-admission), including the fixed byte policy, disclosed hosted-processing risk, canonical requirement/ADR/scenario changes, explicit forward evaluation disposition and bounded shared A0 slice. [Acceptance](plans/completed/m3-04-groq-adapter.md#m304-fit-accept-01--owner-acceptance-and-canonical-reconciliation) resolves the original G2 decision stop; combined G review now passes at [M304-G-FINAL-01](plans/completed/m3-04-groq-adapter.md#m304-g-final-01--accepted-complete-contract), and A0/A/B implementation passes controlled verification and critical reviews. Complete regression, different final review and documentation closure pass. No provider call or later task is authorized.
- **Historical research stop (2026-09-11 UTC):** Primary accepts [M304-G-STOP-01](plans/completed/m3-04-groq-adapter.md#m304-g-stop-01--complete-accounting-evidence-required): two critical research dimensions and mandatory analyst synthesis identify missing Groq-applicable complete counting or a verified finite upper bound. G1/G3 remain provisional, G4 and A/B are pending, and no implementation or provider call occurred. Resume G only with the named authoritative evidence or a coordinated owner-approved fit amendment; requirements and task Verification remain unchanged.
- **Execution activation (2026-09-11 UTC):** The owner authorizes exact M3-04 bounded research, primary literal decisions, delegated implementation, existing-budget corrections, verification and documentation closure. [Refreshed entry](plans/completed/m3-04-groq-adapter.md#m304-entry-02--execution-authorization-and-preserved-baseline) passes strict, focused and frozen-identity checks; G must pass before A/B preflight. Actual Groq/Local generation, provider probes, other tasks, commits, publication and pushing remain excluded. Existing developer secret material is preserved.
- **Planning activation (2026-09-11 UTC):** The owner selected current-project review and [ExecPlan creation](plans/completed/m3-04-groq-adapter.md), not implementation, credential configuration or provider execution. Completed M3-02 supplies the shared contract; M3-03's accepted Local implementation and blocked real capacity remain unchanged. [Entry evidence](plans/completed/m3-04-groq-adapter.md#m304-entry-01--planning-evidence) records fresh strict TypeScript, 149 focused tests and unchanged frozen identities. The [progress record](progress/m3-04-groq-adapter.md) mirrors planning only; secret, accounting and wire literals still require the plan's G barrier before implementation.
...
- **Authorities:** `REQ-LLM-003`–`REQ-LLM-005`, `REQ-LLM-007`, `REQ-LLM-009`, `REQ-LLM-015`, `REQ-LLM-016`, `REQ-LLM-019`, `REQ-LLM-021`, `REQ-INST-017`, `REQ-SEC-005`, `REQ-SEC-013`–`REQ-SEC-016`, ADR-0014, ADR-0020, ADR-0023, SPEC-004, and HS-009.
...
- **Parent milestone / role / status:** M3 / UI, integration, and verification / **In progress**.
- **Planning activation (2026-09-12 UTC):** The owner selected current-state review and ExecPlan creation only. Accepted M3-03 A/B and Complete M3-04 satisfy the amended start condition; M3-03/C remains Blocked. [Planning evidence](plans/m3-05-generation-checkpoint.md#m305-entry-01--planning-evidence) records fresh strict TypeScript, 103 pure tests and unchanged frozen generation inputs. The plan gates integration literals, separate-owner TDD, accessible presentation and actual eligible provider proof. No implementation, model operation, credential inspection, acquisition, other task or Git mutation is authorized by this planning request.
- **Execution plan:** [M3-05 — Generation checkpoint](plans/m3-05-generation-checkpoint.md); [progress summary](progress/m3-05-generation-checkpoint.md).
- **Objective:** Join the shared stage and both real adapters into one accessible explicit-generation flow.
- **Inputs, dependencies, and scheduling:** May start after accepted M3-03 A/B implementation and reviews plus Complete M3-04, under the owner's 2026-09-10 scheduling amendment. M3-03/C may remain pending until the implemented reviewer interface permits the full representative smoke. M3-05 completion still requires every Verification item, including real eligible Local and Groq calls; C supplies Local evidence only if complete evidence identity and coverage match. This start condition selects no task and grants no additional model call. Critical-path integration checkpoint before M4.
...
- **Inputs, dependencies, and scheduling:** May start after an accepted M3-05 implementation/interface checkpoint while M3-05 real-call verification remains pending, under the owner's 2026-09-10 scheduling amendment. Every task's completion requirements remain binding; an implementation checkpoint is not task completion. This start condition selects no task. M4-02 retains its M4-01 dependency; M6-02 retains its passed-capacity and other prerequisites. Critical path.
...
- **Verification:** Exercise all three final actions; confirm support and blocking-judgment gates; validate durable aggregate readback through the application-owned boundary; preserve the original proposal and sibling data; include automated keyboard/announcement checks and manual visual review. User-facing reload restoration remains Deferred through OD-026.
...
- **Carried verification:** Complete the M2-03 full-guidance/detail readability and reflow check at actual 200% browser zoom explicitly deferred by the owner on 2026-09-09. Follow REQ-A11Y-006's visual-only manual method and automated keyboard/focus/announcement checks; do not treat them as spoken-output proof.
...
- **Expected output:** Automated application-accessibility, keyboard, focus and announcement-semantics checks plus one manual visual smoke path over the core workflow and its important status/error states.
...
- **Likely surfaces:** Future automated accessibility/keyboard/announcement checks, visual-review checklist, and recorded limitations.
...
After each group, its named checkpoint must integrate and verify the combined behavior before dependent work begins, except for the explicit owner-accepted start conditions in M3-05 and M4-01. Those exceptions retain every completion proof. Without separate worktrees, execute the tasks serially. No artificial parallelism is created inside one aggregate mutation, shared lifecycle, UI composition, or integration boundary.
...
The spine uses the accepted implementation checkpoints for the two M3-05 and M4-01 starts; full M3-03/C and M3-05 verification remain pending until their required real interfaces and calls can be proved.
...
- The immediate run/scan fields are frozen in the [completed M1-01 literal contract](plans/completed/m1-01-run-and-scan-contracts.md#authored-literal-contract--m101-literals-001-l1). Fields for later aggregate sections, UI layout/copy, and later task-specific package and verification-tool choices remain implementation literals until their owning task selects them.
