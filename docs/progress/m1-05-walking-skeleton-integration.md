...
- **ExecPlan:** [Walking-skeleton integration](../plans/completed/m1-05-walking-skeleton-integration.md)
- **Updated:** 2026-09-03
...
- **Evidence:** The active [M1-05 ExecPlan](../plans/completed/m1-05-walking-skeleton-integration.md) records exact responsibilities, paths, public compatibility, lifecycle invariants, commands, budgets, and stop conditions.
...
- **Evidence:** The active [M1-05 ExecPlan](../plans/completed/m1-05-walking-skeleton-integration.md) records exact responsibilities, stable public imports, serialization and lifecycle constraints, environment, commands, budgets, cleanup checks, and stop conditions.
...
- **Evidence:** The active [M1-05 ExecPlan](../plans/completed/m1-05-walking-skeleton-integration.md) records the exact public compatibility, responsibilities, dependency direction, frozen-policy requirement, safe-inspection invariants, commands, ownership, cleanup, and stop conditions.
...
- **Evidence:** The focused repository suite currently passes 55/55. The active [M1-05 ExecPlan](../plans/completed/m1-05-walking-skeleton-integration.md) records public compatibility, responsibilities, default-filesystem interception, path/identity and publication invariants, commands, ownership, cleanup, and stop conditions.
...
## 2026-09-03 — Public-page smoke accepted

- **Project outcome:** The owner-authorized network-capable execution completed through the unchanged production service, managed browser, persisted aggregate, and existing Results UI. Complete coverage contains all three supported rules, zero automated Findings, and 20 distinct color-contrast scanner-review observations. This is not a certification, all-clear result, or provider call.
- **Evidence:** The live response and retained aggregate passed the exact smoke validator and internal read comparison. The service stopped normally with exit 0; the post-stop tuple, exact aggregate hash, unchanged empty sibling set, empty scratch, and refused service socket passed. The content-safe identity is frozen as [M105-SMOKE-RETAINED-001](../plans/completed/m1-05-walking-skeleton-integration.md#m105-smoke-retained-001--accepted-public-observation).
- **Scope and retention:** Earlier sandbox failures remain non-reusable history. No source, test, dependency, configuration, or Git state changed for the successful execution. Public target/content and screenshots are excluded from tracked documentation; the local disposable aggregate and exact build remain intact for review.
- **Next boundary:** Different fresh integrated critical review, then exact artifact cleanup and documentation closure. M1-05 remains In progress; M2, commit, and push remain unauthorized.

## 2026-09-03 — Walking-skeleton integration checkpoint accepted

- **Project outcome:** The production entry now serves the exact built client from the enumerated loopback origin. Its same-origin `POST /api/runs` boundary sends only target plus mode, invokes the real scanner, publishes one validated durable `run.json`, and returns the bounded outcome to the existing Results surface. API-only construction remains read-only. The closed asset loader rejects linked descendant components and prevents callers from mutating its private startup bytes.
- **Agent-workflow outcome:** Worker-first Red/Green execution included only owner-authorized bounded recoveries. The first complete S3 review returned `REVISE` with two Major asset-loader findings; the exact test Red and one-file production Green corrected both. A different fresh S3 reviewer then returned PASS with no Blocker, Major, or Minor finding and accepted all seven changed application files as `RETAINED`. The earlier wrong-digest worker-packet process finding remains recorded as historical evidence.
- **Evidence:** The corrected complete barrier passes 324/324 runtime cases: 58 contract, 55 repository, 69 local service, 20 normalization, 88 real scanner, four walking skeleton, and 30 UI. Strict TypeScript, six changed-server syntax checks, the seven-receipt frozen tuple, exact response bytes, linked-ancestor rejection, mutation isolation, and clean scratch/cache checks all pass. The retained client build and pinned browser runtime remain frozen.
- **Documentation impact:** The active ExecPlan, this task record, and the progress index now report the accepted controlled integration checkpoint. Requirements, architecture decisions, persistence schema, scanner policy, provider lifecycle, and roadmap status do not change.
- **Next boundary:** M1-05 remains In progress at the owner-gated trusted public HTTPS smoke. The smoke requires the exact authorized URL and an explicit declaration that its generated run is disposable; final integrated review, cleanup, documentation closure, commit, and push remain outstanding or unauthorized.

## 2026-09-03 — First public smoke observation rejected and cleaned

- **Project outcome:** The first owner-approved public page could not be opened by the managed scanner. This mutable target/network observation does not establish a product defect and does not satisfy the required completed public smoke.
- **Evidence:** The application produced one validated Local-mode failed aggregate with failure category `navigation`. The smoke validator correctly rejected its non-completed status, and the production service then stopped cleanly. Exact cleanup rechecked the sole ordinary one-file run and SHA-256 before removing only that disposable run; `data/runs` is empty again. The private target is not retained in project documentation.
- **Documentation impact:** This record and the active ExecPlan preserve the non-reusable observation and cleanup without changing requirements, architecture, implementation status, or the accepted controlled integration evidence.
- **Next boundary:** One owner-directed replacement target is permitted. It requires a new exact public HTTPS URL, permission/trust and non-sensitive redirect confirmation, and a fresh disposable-run declaration. No automatic retry, commit, or push is authorized.

## 2026-09-03 — Replacement smoke blocked by sandbox network policy

- **Project outcome:** The sole owner-directed replacement also reached the real managed-browser navigation boundary but could not open the approved public page. The public smoke remains unpassed; the accepted controlled integration checkpoint is unchanged.
- **Diagnosis:** The validated aggregate again reports Local-mode failure category `navigation` with clean browser teardown. A bounded diagnostic using the identical managed Chromium configuration captured `net::ERR_NETWORK_ACCESS_DENIED`. A read-only HTTPS HEAD request outside the Codex sandbox returned 200 for the same page, isolating the failure to the sandbox's outbound browser restriction rather than the page or application behavior.
- **Evidence and cleanup:** The completed-status validator rejected the failed aggregate and the production service stopped cleanly. Exact cleanup rechecked the sole ordinary one-file run and SHA-256 before deleting only that disposable replacement run. `data/runs` and task scratch are empty, and no source, test, dependency, configuration, accepted build, or Git state changed.
- **Next boundary:** Both permitted smoke targets are consumed. M1-05 remains In progress pending explicit owner reconciliation and a network-capable smoke execution boundary. Final integrated review, archival, and task completion cannot proceed from this failed external observation.

## 2026-09-03 — Network-capable same-target correction authorized

- **Owner direction:** The owner explicitly requested one rerun outside the sandbox after reviewing the exact `net::ERR_NETWORK_ACCESS_DENIED` diagnosis.
- **Execution boundary:** The replacement target, Local mode, disposable-run declaration, frozen source/test/build/runtime tuple, and exact smoke commands remain unchanged. The first outside-sandbox wrapper stopped before service startup because the host process could not read an ignored lease receipt. The reconciled route runs the exact tuple and scratch gate inside the sandbox immediately before and after, then runs only PREP plus the unchanged production start/stop outside it so the managed Chromium child can use outbound network access. Validation, retained evidence, review, and cleanup remain controlled by the existing plan.
- **Scope guard:** This is an environment correction, not a third target, product change, provider call, or automatic retry. Any failure stops. M1-05 remains In progress until the smoke and later closure gates pass.

## 2026-09-03 — Final review, cleanup, and closure

- **Project outcome:** M1-05 is Complete. The working same-origin scan-to-disk path, all 324 tests, independent strict TypeScript, required syntax checks, and authorized public-page smoke passed. The smoke completed in Local mode with zero Findings and 20 distinct color-contrast manual-review observations; no provider was called and no accessibility verdict is implied.
- **Review:** Different fresh critical review `M105-FINAL-INTEGRATED-REVIEW-01` returned PASS WITH FOLLOW-UPS: no Blocker or new actionable Major, all seven changed modules RETAINED, and one Minor for a missing revision entry. Primary added that entry and passed focused documentation validation before accepting review. Historical failed evidence and the wrong-digest packet finding remain preserved.
- **Cleanup:** The exact frozen command removed only the authorized disposable smoke run, exact client build, and three empty scratch roots. Run siblings, retained browser runtime, dependencies, shared caches, source, tests, protected inputs, and seven terminal receipts remained unchanged. The service was already stopped.
- **Documentation impact:** Reconciled the public/developer overview, authority and architecture indexes, roadmap, concept/context, delivery readiness, lifecycle model, plan index, and progress owners; archived the task plan with repaired links. No requirement or ADR semantics changed. Post-archive closure validation passed exact identities, residue, task totals, 86 Markdown files / 1,892 links / 529 fragments, 21 PowerShell examples, UTF-8, and whitespace. An LF-only heading-check false rejection was corrected to accept CRLF without removing any required heading.
- **Next boundary:** M2-01 remains Not started and requires explicit owner selection. Retrieval, generation, human proposal review, comparison, commit, and push are not authorized by this closure.

## 2026-09-03 — Owner-authorized post-closure correction

- **Project outcome:** M1-05 is reopened. At committed baseline `1f92fe89e28fb2e645a97875ee38390d4744a858`, primary reproduced unquoted stylesheet references bypassing the closed asset table and five abort-helper executions reaching no HTTP request or stream event. Earlier complete-suite and public-smoke observations remain historical; they do not prove these missing checks.
- **Scope:** Correct only `client-assets.ts`, `walking-skeleton.test.ts`, and its existing harness, plus primary-owned task/status documentation. Preserve the Accepted closed-asset and no-side-effect stream contracts, provider isolation, dependencies, evaluation fixtures, public interfaces, and prior history.
- **Workflow:** Read-only test preflight; guarded test-owned Red with passing characterization of the previously uncovered abort path; separate one-file production Green; complete replacement verification; fresh S3 and different final integrated review; exact generated-artifact cleanup and renewed documentation closure.
- **Next boundary:** Preflight and the active ExecPlan's bounded correction contract. No new public smoke, provider call, next roadmap task, commit, or push is authorized.

## 2026-09-03 — Post-closure Red, Green, and complete verification

- **Project outcome:** Both bounded corrections are implemented. Eleven added stylesheet cases establish quoted/mixed/unquoted CSS loading and fail-before-storage/listen negatives. The repaired abort helper proves real partial data, actual aborted/error events, no completed-body intake dispatch, unchanged scanner/storage/sibling state, and responsive health.
- **Workflow:** Test-worker preflight PARTIAL isolated the missing asset behavior. The guarded test turn produced 15 cases / 5 pass / 10 intended stylesheet failures with passing abort characterization. A separate guarded one-file code turn reproduced Red and reached 15/15 Green without changing the tests; primary accepted both compliant receipts and RETAINED production responsibility.
- **Verification:** The complete seven-file barrier passed 335/335, independent strict TypeScript and all three syntax checks passed, corrected source/test identities were frozen, and build/runtime/evaluation/fixtures remained exact. Scratch/run roots are empty and Vite caches absent. The original public smoke remains historical and was not repeated.
- **Review:** Fresh `M105-PC08-S3-01` returned PASS WITH FOLLOW-UPS, with no Blocker/Major and RETAINED cohesion. Independent checks passed 25 tokenizer cases, three real aborts, a deliberate no-arrival timeout, restored hooks, strict/syntax and exact identities. Primary consolidated the duplicate progress checkpoint identified as the sole Minor.
- **Next boundary:** Different final integrated review, exact build/empty-scratch cleanup, and renewed documentation closure. M1-05 remains In progress.


## 2026-09-03 — Post-closure correction and renewed closure

- **Project outcome:** Both P2 corrections are accepted. Different fresh final review `M105-PC08-FINAL-01` returned PASS with no remaining findings and RETAINED cohesion. Independent integrated asset/HTTP/abort/control checks passed; all 335 tests and strict/syntax evidence remain identity-valid.
- **Cleanup:** Exact correction cleanup removed only the verified regenerated `dist/client` and three empty test scratch roots. No run was deleted. The retained browser, source/tests, protected inputs, evaluation/fixtures, both correction receipts, empty run siblings, Git endpoint and empty index are unchanged. The removed generated state is reproducible through the README setup.
- **Documentation impact:** Reconciled all 13 documentation/status owners, restored M1-05 Complete, archived the same plan, and repaired navigation. Final post-mutation closure passed exact identities and residue checks, 16 changed paths, eight Complete / 20 Not started tasks, 86 Markdown files, 1,892 links, 529 fragments, 26 PowerShell examples, required headings, UTF-8 and whitespace. No requirement or ADR semantics changed; original failures and public smoke remain historical.
- **Next boundary:** M2-01 remains Not started and requires explicit owner selection; no commit or push is authorized.
