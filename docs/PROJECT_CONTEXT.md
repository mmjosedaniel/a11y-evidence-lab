...
The primary MVP user is a frontend developer investigating findings from one authorized public page and preparing remediation decisions one selected finding at a time. A QA engineer is the secondary user who can reproduce a finding and verify the later result. The single-user MVP does not introduce accounts, product roles, permissions, assignments, or collaboration features.
...
The selected first portfolio slice accepts one user-entered, explicitly authorized public HTTPS page per analysis. One atomic provider-independent scan runs exactly `image-alt`, `label`, and `color-contrast`, and creates one independent finding for every violation node returned within the accepted safety and resource envelope. Native axe `incomplete` observations remain separate from violations. A successful zero-finding result requires complete coverage of all three rules; a bound overflow or coverage-incomplete scan fails visibly and is never reported as a truncated success. The user then selects one finding at a time for retrieval, evidence-sufficiency evaluation, proposal generation or abstention, human review, and later comparison. This fixed-cardinality rule suite and sequential child workflow add useful evidence variety without crawling, bulk generation, a queue, or workflow orchestration.

The three project-owned synthetic controlled profiles—`informative-image-alt`, `form-input-label`, and `text-contrast`—remain the frozen evaluation baseline for the three mappings, including controlled comparison outcomes. They are evaluation inputs, not user-submitted runtime page inputs. [OD-020](requirements/DELIVERY_READINESS_AND_OPEN_DECISIONS.md#od-020--authorized-public-page-analysis-scope) records the current scope and its relationship to OD-019. Authenticated targets, non-public destinations, link discovery, crawling, multi-page intake, broader rules, and bulk generation remain excluded.
...
The portfolio MVP is a browser-delivered local application, not an installed desktop product. The developer starts one local application service and opens its loopback address in Chrome or Edge. Browser-delivered code remains an unprivileged interface; the service owns local files, model-runtime and Groq calls, constrained public-page browser automation, and other privileged operations. Live target content is untrusted. [ADR-0017](architecture/decisions/ADR-0017-authorized-public-page-scan-boundary.md) requires HTTPS-only intake, public-address and redirect validation, an ephemeral isolated browser context, and explicit scan-egress controls. Exact numeric resource limits remain open planning details and are not silently accepted by this summary.
...
- The runtime product would operate only on one user-attested, policy-valid public HTTPS page per analysis; public reachability does not replace the user's authorization attestation.
- The scan would execute exactly `image-alt`, `label`, and `color-contrast`, retain all in-envelope violation nodes, and distinguish a valid zero from native axe `incomplete` observations and coverage-incomplete/failed scanning.
- The fixed synthetic scenarios would remain evaluation inputs, not user-submitted runtime targets.
- The MVP would not accept authentication state or non-public destinations, discover or follow links as scan targets, crawl, batch pages, broaden the rule set, or bulk-generate proposals.
...
The project remains in idea exploration. No implementation has started. [OD-020](requirements/DELIVERY_READINESS_AND_OPEN_DECISIONS.md#od-020--authorized-public-page-analysis-scope) accepts the authorized-public-page portfolio boundary and supersedes OD-019 only where that earlier decision limited runtime input to synthetic profiles and processed one profile per scan; OD-019's three fixtures remain the evaluation baseline. The primary and secondary users, global Local/Groq analysis mode, Groq as the only external provider, no-fallback behavior, local browser startup, and no-installer boundary are accepted at their recorded scopes. [ADR-0017](architecture/decisions/ADR-0017-authorized-public-page-scan-boundary.md) records the mandatory public-page trust boundary. The initial technology baselines—including TypeScript, React, and the bounded LangChain role—remain evaluation choices rather than release-qualified dependencies. LangGraph, LangSmith, hosted tracing, formal release qualification, desktop packaging, and installer work are deferred. Exact numeric scan limits, implementation commands, package versions, and the capacity-screened local model configuration still require evaluation before development claims can be made.
