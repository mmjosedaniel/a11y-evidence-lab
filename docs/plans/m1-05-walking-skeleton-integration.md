...
Both behavior-preserving structural extractions and their verification barriers are complete. `service.ts` remains the stable public entry point and keeps the complete ordering-sensitive service lifecycle; four focused internal modules own its stateless responsibilities. `scan-page.ts` and `normalize-scan.ts` remain the stable scanner entry points and retain their ordering-sensitive coordinators; five focused internal modules own profile, request, capture, native-reader, and rule-projection responsibilities. Fresh S3 review passed each slice. M1-05 remains In progress because real HTTP scanner integration and the milestone-level end-to-end browser/build checkpoint remain outstanding.
