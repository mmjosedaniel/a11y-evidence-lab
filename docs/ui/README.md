...
- **Status:** M1-04 Analyze/Results and M2-03 guidance retain their recorded verification limits. M3-05 adds generation and original proposal presentation, with current verification status in its owning plan; human review and comparison remain planning guidance.
...
| Selected-finding workspace | Human-readable Finding identity, deterministic explanation, affected element, page location, and direct rule-specific evidence. M2-03 adds explicit guidance, complete citations/notices, evidence sufficiency, supported eligibility, abstention and retrieval failure. M3-05 adds explicit generation, truthful invocation status and the original proposal. Human review remains later work. Only one FindingWorkflow is selected at a time; sibling states remain visible and unchanged. | After a Finding is selected |
...
- M2-03 exposes **Get guidance** only for Findings and preserves explicit activation. M3-05 adds **Generate** for a supported selected Finding with the exact live continuation. A provider or validation failure remains visible and never produces automatic retry, mode switching, or fallback.
